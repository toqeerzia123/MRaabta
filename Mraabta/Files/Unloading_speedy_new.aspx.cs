...
{
    public partial class Unloading_speedy_new : System.Web.UI.Page
    {
                    ...
                    if (detail.Rows.Count > 0)
                    {
                        ...
                        if (detail.Rows.Count != 0)
                        {
                            if (detail.Rows[0]["isunloaded"].ToString() == "1")
                            {
                                ...
                                /***********START UNLOADING EDITING WORK BY TALHA 15 DEC 202************/
                                ...
                                dt = GetUnLoadedBags(clvar);
                                ViewState["bags"] = dt;
                                dtC = GetUnloadedConsignment(clvar);
                                ViewState["cns"] = dtC;
                                /***********END UNLOADING EDITING WORK BY TALHA 15 DEC 202************/
                            }
                            else
                            {
                                txt_vid.Enabled = false;
                        ...
                        if (dt != null || dtC != null)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                hd_master.Value = dt.Rows[0]["UnLoadingID"].ToString();
                            ...
                            if (dtC.Rows.Count > 0)
                            {
                                hd_master.Value = dtC.Rows[0]["UnLoadingID"].ToString();
        ...
        public static DataSet GetUnLoadingData(string ULID)
        {

            string sqlString = @"select BagNumber from mnp_UnloadingBag where UnLoadingID = '" + ULID+ @"'
                                 select ConsignmentNumber from mnp_UnloadingConsignment where UnLoadingID = '" + ULID+"'";
        ...
        public static string SaveChangedValues_saveButton(string unloadingID, BagModel[] Bags, ConsignmentModel[] Consignments)
        {
            ...
            DataSet dsUnloading;
            dsUnloading = GetUnLoadingData(unloadingID);
            if (Bags.Count() > 0)
            {
                foreach (BagModel Bag in Bags)
                {
                    //dtUnloading.Select("BagNumber = '"+ Bag.BagNo + "'");
                    if (dsUnloading.Tables[0].Rows.Count == 0 || dsUnloading.Tables[0].Select("BagNumber = '"+ Bag.BagNo + "'").Count() == 0)
                    {
            ...
            if (Consignments.Count() > 0)
            {
                ...
                {
                    if (dsUnloading.Tables[1].Rows.Count == 0 || dsUnloading.Tables[1].Select("ConsignmentNumber = '" + Consignment.ConNumber + "'").Count() == 0)
                    {
                        //if (Consignment.hd_cnStatus.ToUpper() != "RECEIVED")
                        //{
        ...
        public DataTable GetLoadingBags(Cl_Variables clvar)
        {
            ...
            string sqlString = @"SELECT dbo.TRIM(LB.BAGNUMBER) BAGNO, '' DESCRIPTION, CASE WHEN (LB.BAGWEIGHT IS NULL OR lb.BagWeight = '') THEN b.totalWeight ELSE lb.BagWeight end  TOTALWEIGHT, B1.SNAME ORIGINBRANCHID, LB.BAGORIGIN ORIGINID,
                                LB.BAGDESTINATION DESTID, B2.SNAME DESTBRANCHID, CASE WHEN ISNULL(B.STATUS, 0) = 0 THEN 'Bagged' WHEN B.STATUS = 1 THEN 'Unbagged' END STATUS, 
                                CASE WHEN (LB.BAGSEAL IS NULL OR lb.BagSeal = '') THEN b.sealNo ELSE lb.BagSeal END SEALNO, LB.REMARKS REMARKS, 'SHORT RECEIVED' BAGSTATUS, LB.ULOADINGSTATEID DESCSTATUS
                                --, case when ulb.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulb.UnloadingStateID = 7 then 'EXCESS RECEIVED' else 'RECEIVED' end as state, 
                                , case when ulb.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulb.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulb.UnloadingStateID = 5 then 'RECEIVED' when ulb.BagNumber is null then 'SHORT RECEIVED' end as state, 
                                ulb.UnloadingStateID, ulb.bagRemarks, isnull(ulb.UnLoadingID, '0') as UnLoadingID
                                FROM MNP_LOADING L
                                INNER JOIN MNP_LOADINGBAG LB ON LB.LOADINGID = L.ID
                                LEFT OUTER JOIN BAG B ON B.BAGNUMBER = LB.BAGNUMBER
                                INNER JOIN BRANCHES B1 ON B1.BRANCHCODE = LB.BAGORIGIN
                                INNER JOIN BRANCHES B2 ON B2.BRANCHCODE = LB.BAGDESTINATION
                                LEFT OUTER JOIN MNP_UNLOADING UL ON UL.REFLOADINGID = CAST(L.ID AS NVARCHAR)
                                left join mnp_UnloadingBag ulb on ulb.UnloadingID = ul.id and ulb.BagNumber = lb.bagNumber
                                WHERE L.ID = '" + clvar.LoadingID + "'";
        ...
        public DataTable GetUnLoadedBags(Cl_Variables clvar)
        {

//            string sqlString = @"SELECT dbo.TRIM(ulb.BAGNUMBER) BAGNO, case when ulb.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulb.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulb.UnloadingStateID = 5 then 'RECEIVED' 
//                                when ulb.BagNumber is null then 'SHORT RECEIVED' end DESCRIPTION, CASE WHEN (ulb.BAGWEIGHT IS NULL OR ulb.BagWeight = '') THEN b.totalWeight ELSE ulb.BagWeight end  TOTALWEIGHT
//                                , B1.SNAME ORIGINBRANCHID, ulb.BAGORIGIN ORIGINID,ulb.BAGDESTINATION DESTID, B2.SNAME DESTBRANCHID, 
//                                CASE WHEN ISNULL(B.STATUS, 0) = 0 THEN 'Bagged' WHEN B.STATUS = 1 THEN 'Unbagged' END STATUS, 
//                                CASE WHEN (ulb.BAGSEAL IS NULL OR ulb.BagSeal = '') THEN isnull(b.sealNo,'') ELSE ulb.BagSeal END SEALNO, ulb.bagRemarks REMARKS, case when ulb.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulb.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulb.UnloadingStateID = 5 then 'RECEIVED' 
//when ulb.BagNumber is null then 'SHORT RECEIVED' end BAGSTATUS, ulb.unloadingStateID DESCSTATUS
//                                , case when ulb.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulb.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulb.UnloadingStateID = 5 then 'RECEIVED' 
//                                when ulb.BagNumber is null then 'SHORT RECEIVED' end as state, 
//                                ulb.UnloadingStateID, ulb.bagRemarks, isnull(ulb.UnLoadingID, '0') as UnLoadingID
//                                FROM MNP_LOADING L
//                                Inner JOIN MNP_UNLOADING UL ON UL.REFLOADINGID = CAST(L.ID AS NVARCHAR)
//                                Inner join mnp_UnloadingBag ulb on ulb.UnloadingID = ul.id 
//                                INNER JOIN BRANCHES B1 ON B1.BRANCHCODE = ULB.BAGORIGIN
//                                INNER JOIN BRANCHES B2 ON B2.BRANCHCODE = ULB.BAGDESTINATION
//                                LEFT OUTER JOIN BAG B ON B.BAGNUMBER = ULB.BAGNUMBER
//                                WHERE L.ID = '" + clvar.LoadingID + "'";
            ...
            string sqlString = @"SELECT dbo.TRIM(ulb.BAGNUMBER) BAGNO, case when ulb.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulb.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulb.UnloadingStateID = 5 then 'RECEIVED' 
                                ...
                                Inner JOIN MNP_UNLOADING UL ON UL.REFLOADINGID = CAST(L.ID AS NVARCHAR)
                                Inner join (
                                    select (select id from mnp_unloading where RefLoadingID = '" + clvar.LoadingID + @"') as UnloadingID, lb.BagNumber, destination as BagDestination, lb.CreatedBy, lb.Createdon, isnull(LB.UloadingStateID, 6) as unloadingStateID, bagWeight, Remarks as bagRemarks, bagOrigin, BagSeal, NULL as ModifiedBy, NULL as ModifiedOn FROM MNP_LOADING L
                                    INNER JOIN MNP_LOADINGBAG LB ON LB.LOADINGID = L.ID where L.ID = '" + clvar.LoadingID + @"' and lb.bagNumber not in (select bagNumber from mnp_UnloadingBag where UnLoadingID = (select id from mnp_unloading where RefLoadingID = '" + clvar.LoadingID + @"'))
                                    union
                                    select * from mnp_UnloadingBag where UnLoadingID = (select id from mnp_unloading where RefLoadingID = '" + clvar.LoadingID + @"')
                                ) ulb on ulb.UnloadingID = ul.id 
                                INNER JOIN BRANCHES B1 ON B1.BRANCHCODE = ULB.BAGORIGIN
                                INNER JOIN BRANCHES B2 ON B2.BRANCHCODE = ULB.BAGDESTINATION
                                LEFT OUTER JOIN BAG B ON B.BAGNUMBER = ULB.BAGNUMBER
                                WHERE L.ID = '" + clvar.LoadingID + "'";
        ...
        public DataTable GetConsignmentForUnload(Cl_Variables clvar)
        {
            ...
            string sqlString = @"select cnNo, Description, weight, pieces, OriginBranchId, OriginID, DestID, DestBranchId, Remarks, CNSTATUS, state, 
                                isnull(UnloadingStateID, case when state = 'RECEIVED' then 5 when state = 'SHORT RECEIVED' then 6 when state = 'EXCESS RECEIVED' then 7 end) as UnloadingStateID, cnRemarks, UnLoadingID 
                                from (
                                select lc.consignmentNumber cnNo, '' Description, lc.CNWeight weight, lc.cnPieces pieces, b1.name OriginBranchId, l.origin OriginID, lc.cnDestination DestID, b2.name DestBranchId,
                                lc.Remarks Remarks, 'SHORT RECEIVED' CNSTATUS , case when ulc.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulc.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulc.UnloadingStateID = 5 then 'RECEIVED' when ulc.ConsignmentNumber is null then 'SHORT RECEIVED' end as state, 
                                ulc.UnloadingStateID, ulc.cnRemarks, isnull(ulc.UnLoadingID, '0') as UnLoadingID
                                from mnp_Loading l
                                inner join MnP_LoadingConsignment lc on lc.LoadingId = l.id
                                inner join Branches b1 on b1.branchCode = l.origin
                                inner join Branches b2 on b2.branchCode = lc.cndestination
                                left outer join mnp_unloading ul on ul.RefLoadingID = CAST(l.id as nvarchar)
                                left join mnp_UnloadingConsignment ulc on ulc.UnLoadingID = ul.id and ulc.ConsignmentNumber = lc.consignmentNumber
                                where l.id = '" + clvar.LoadingID + "') as xb";
        ...
        public DataTable GetUnloadedConsignment(Cl_Variables clvar)
        {

            //string sqlString = @"select cnNo, Description, weight, pieces, OriginBranchId, OriginID, DestID, DestBranchId, Remarks, CNSTATUS, state, 
            //                    isnull(UnloadingStateID, case when state = 'RECEIVED' then 5 when state = 'SHORT RECEIVED' then 6 when state = 'EXCESS RECEIVED' then 7 end) as UnloadingStateID, cnRemarks, 
            //                    UnLoadingID 
            //                    from (
            //                    select ulc.consignmentNumber cnNo, case when ulc.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulc.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulc.UnloadingStateID = 5 then 'RECEIVED' when ulc.ConsignmentNumber is null then 'SHORT RECEIVED' end  Description, ulc.CNWeight weight, ulc.cnPieces pieces, b1.name OriginBranchId, l.origin OriginID, ulc.cnDestination DestID, 
            //                    b2.name DestBranchId, ulc.cnRemarks Remarks, case when ulc.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulc.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulc.UnloadingStateID = 5 then 'RECEIVED' when ulc.ConsignmentNumber is null then 'SHORT RECEIVED' end  CNSTATUS , 
            //                    case when ulc.UnloadingStateID = 6 then 'SHORT RECEIVED' when ulc.UnloadingStateID = 7 then 'EXCESS RECEIVED' when ulc.UnloadingStateID = 5 then 'RECEIVED' when ulc.ConsignmentNumber is null then 'SHORT RECEIVED' end as state, 
            //                    ulc.UnloadingStateID, ulc.cnRemarks, isnull(ulc.UnLoadingID, '0') as UnLoadingID
            //                    from mnp_Loading l
            //                    inner join mnp_unloading ul on ul.RefLoadingID = CAST(l.id as nvarchar)
            //                    inner join mnp_UnloadingConsignment ulc on ulc.UnLoadingID = ul.id and ulc.ConsignmentNumber = ulc.consignmentNumber
            //                    inner join Branches b1 on b1.branchCode = l.origin
            //                    inner join Branches b2 on b2.branchCode = ulc.cndestination
            //                    where l.id = '" + clvar.LoadingID + "') as xb";
            ...
            string sqlString = @"select cnNo, Description, weight, pieces, OriginBranchId, OriginID, DestID, DestBranchId, Remarks, CNSTATUS, state, 
                                ...
                                from mnp_Loading l
                                inner join mnp_unloading ul on ul.RefLoadingID = CAST(l.id as nvarchar)
                                inner join (select (select id from mnp_unloading where RefLoadingID = '" + clvar.LoadingID + @"') as UnLoadingID, lc.ConsignmentNumber, lc.CNDestination, lc.CreatedBy, lc.Createdon, lc.UnloadingStateID, lc.cnPieces, lc.cnWeight, Remarks as cnRemarks, l.origin as CnOrigin, NULL as ModifiedBy, NULL as ModifiedOn   
                                from MnP_LoadingConsignment lc
                                inner join mnp_Loading l on l.id = lc.loadingId
                                left join mnp_unloading ul on ul.RefLoadingID = CAST(l.id as nvarchar)
                                left join mnp_UnloadingConsignment ulc on ul.id = ulc.UnLoadingID and ulc.ConsignmentNumber = lc.consignmentNumber
                                where loadingId = '" + clvar.LoadingID + @"' and ulc.ConsignmentNumber is null
                                union
                                select * from mnp_UnloadingConsignment where UnLoadingID = (select id from mnp_unloading where RefLoadingID = '" + clvar.LoadingID + @"')) ulc on ulc.UnLoadingID = ul.id and ulc.ConsignmentNumber = ulc.consignmentNumber
                                inner join Branches b1 on b1.branchCode = l.origin
                                inner join Branches b2 on b2.branchCode = ulc.cndestination
                                where l.id = '" + clvar.LoadingID + "') as xb";
        ...
        public static string save_BAG(inputsValuesModel inputsValues, BagModel Bag)
        {
            ...
            if (string.IsNullOrEmpty(Bag.Description) || Bag.Description == "undefined")
            {
                Bag.Description = "EXCESS RECEIVED";
            }
        ...
        public static List<string> PerformUnloading_BAG(Cl_Variables clvar, Variable clvar_, BagModel bags, string sealNum, string hd_master)
        {
            ...
            try
            {
                ...
                //{
                query = "delete from mnp_unloadingBag unloadingID = '" + unloadingID + "' and bagNumber = '" + bags.BagNo + "'";
        ...
        public static string save_CN(inputsValuesModel inputsValues, ConsignmentModel CN)
        {
            ...
            if (CN.Description == "undefined")
            {
                CN.Description = "EXCESS RECEIVED";
            }
        ...
        public static List<string> PerformUnloading_CN(Cl_Variables clvar, Variable clvar_, ConsignmentModel CN, string sealNum, string hd_master)
        {
            ...
            try
            {
                ...
                if (cnStatus.ToUpper() == "SHORT RECEIVED")
                {
                    query = "Delete from MnP_UnLoadingConsignment where UnLoadingID = '" + unloadingID + "' and ConsignmentNumber = '" + clvar.consignmentNo + "'";
                    queries.Add(query);
