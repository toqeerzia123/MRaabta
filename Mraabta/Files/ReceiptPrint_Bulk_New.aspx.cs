...
{
    public partial class ReceiptPrint_Bulk_New : System.Web.UI.Page
    {
        ...
        public DataTable GetRunsheetHeaderDetails(Cl_Variables clvar)
        {
            ...
            //  + "	WHERE  r.runsheetNumber = '202114903759' and r.branchCode = '4'";
        ...
        public DataTable GetBulkCODRRByRunsheetNumber(Cl_Variables clvar)
        {
            ...
            string sql = "SELECT ROW_NUMBER() OVER(ORDER BY ISNULL(dbo.splitString(pv.depositSlipNo, '-', '2'),0) DESC ) Sr,\n"
                ...
                // + "WHERE  rc.runsheetNumber = '202114903759' AND rc.BranchCode = '4'\n"
            ...
            sql = $@"SELECT ROW_NUMBER() OVER(ORDER BY ISNULL(dbo.splitString(x.depositSlipNo, '-', '2'),0) DESC ) Sr,
x.CNNo,x.AccountNumber,x.AccName,x.CodAmount,x.VoucherID,x.RRNumber,x.RRDate,x.RREntry,x.RRAmount,x.RefNumber,
case when x.RRNumber IS NOT NULL THEN 'Y' ELSE 'N'   END VoucherExists,
x.RunsheetNumber,x.SourceWithAmount,x.RunsheetReason,x.RiderCode,x.ExpressCenterCode,x.CreditClientID,
 ISNULL(dbo.splitString(x.depositSlipNo+'-1', '-', '2'),0) Attempt,
case when x.voucherid is null then 'R' else 'E' end RRstatus,
x.DepositSlipNo,x.CODType,x.SortOrder,x.PaymentSourceId
from
...
STUFF((SELECT distinct ', ' +cast(p2.id as varchar) 
         ...
         from PaymentVouchers p2  where p2.ConsignmentNo=c.consignmentNumber FOR XML PATH(''))  ,1,2,'')             RRNumber, 
                   (SELECT top 1 cast(p2.VoucherDate as varchar)
        ...
        public void PrintBulkCODRR(DataTable header, DataTable dt)
        {
            ...
            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
            {
                ...
                PdfPTable table_detail = new PdfPTable(12);
                table_detail.TotalWidth = 550f;
                table_detail.SetWidths(new float[12] { 15f, 37f, 70f, 26f, 98f, 25f, 45f, 88f, 20f, 36f, 45f, 45f });
                ...
                cell = PhraseCell(new Phrase("Source(Amount)", FontFactory.GetFont("Calibri", 7, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)), PdfPCell.ALIGN_LEFT);
                cell.Colspan = 1;
                cell.Padding = 2f;
                cell.BorderColorTop = BaseColor.BLACK;
                cell.BorderColorLeft = BaseColor.BLACK;
                ...
                for (int j = 0; j < ds.Tables.Count; j++)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        cell = PhraseCell(new Phrase(dr["SourceWithAmount"].ToString(), FontFactory.GetFont("Calibri", 7, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)), PdfPCell.ALIGN_LEFT);
                        cell.Colspan = 1;
                        cell.Padding = 2f;
                        //cell.BorderColorTop = BaseColor.BLACK;
                        cell.BorderColorLeft = BaseColor.BLACK;
                    ...
                    if (j == 0)
                    {
                        ...
                        cell = PhraseCell(new Phrase("Source", FontFactory.GetFont("Calibri", 7, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)), PdfPCell.ALIGN_LEFT);
                        cell.Colspan = 1;
                        cell.Padding = 2f;
                        cell.BorderColorTop = BaseColor.BLACK;
                        cell.BorderColorLeft = BaseColor.BLACK;
                    ...
                    else
                    {
                        ...
                        cell = PhraseCell(new Phrase("SourceWithAmount", FontFactory.GetFont("Calibri", 7, iTextSharp.text.Font.BOLD, iTextSharp.text.BaseColor.BLACK)), PdfPCell.ALIGN_LEFT);
                        cell.Colspan = 1;
                        cell.Padding = 2f;
                        cell.BorderColorTop = BaseColor.BLACK;
                        cell.BorderColorLeft = BaseColor.BLACK;
