...
{
    public partial class BookingEC : System.Web.UI.Page
    {
        CommonFunction CF = new CommonFunction();
        Cl_Variables clvar = new Cl_Variables();
        Consignemnts con = new Consignemnts();
        //    Cl_Invocie inv_ = new Cl_Invocie();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("~/Files/PickupRider_ExpressCenter.aspx");
            }
        ...
        protected void btn_SaveConsignment_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Session["BookingUserStatus"].ToString() == "1")
                {
                    seq = SequenceCheck(clvar, " AND p.MasterProduct= (select products from serviceTypes_new where serviceTypeName = '" + cb_ServiceType.SelectedValue + "')");
                }
                else
                {
                    seq = SequenceCheck_Old(clvar, " AND mzc.Product = (select products from serviceTypes_new where serviceTypeName = '" + cb_ServiceType.SelectedValue + "')");
                }
                ...
                if (error == "")
                {
                    new Thread(() =>
                    {
                        ...
                        try
                        {
                            ...
                            HttpResponseMessage response = client.PostAsJsonAsync("http://20.46.47.21/CustomerAPI/api/Users/SendMessageAndroid", new PickupRiderNotification { U_ID = Session["U_ID"].ToString(), TicketNumber = Request.QueryString["TicketNumber"].ToString(), CallStatus = "4" }).Result;
                    ...
                    if (dd_discount.SelectedValue == "0")
                    {
                        ...
                        //string script = String.Format(script_, "invoicepdf_2.aspx?id=" + txt_ConNo.Text, "_blank", "");
        ...
        public string Add_Consignment_Discount(Cl_Variables obj, float denseWeight, string remarks)
        {
            ...
            try
            {
                ...
                sqlcmd.Parameters.AddWithValue("@createdBy", obj.createdBy);
                //sqlcmd.Parameters.AddWithValue("@status", obj.status);
                sqlcmd.Parameters.AddWithValue("@status", "9");
                sqlcmd.Parameters.AddWithValue("@totalAmount", obj.TotalAmount);
                sqlcmd.Parameters.AddWithValue("@zoneCode", obj.Zone);
                sqlcmd.Parameters.AddWithValue("@branchCode", obj.origin);
                sqlcmd.Parameters.AddWithValue("@expressCenterCode", obj.expresscenter);
        ...
        //public string Add_Consignment_Discount_Tariff(Cl_Variables obj, float chargeAmount_, float gst_, float discountAmount_, float discountGst_)
        ...
        public DataTable GetECBySequence(string consignmentNumber)
        {
            ...
            if (Session["BookingUserStatus"].ToString() == "1")
            {
                sqlString = @"SELECT s.ECRiderCode ExpressCenter, s.ECRiderCode ECName,*
                FROM   MnP_CNIssue_Stock s
                INNER JOIN MnP_CNIssue_Product p ON s.ProductID = p.ID
        ...
        public DataSet PriceModifiers()
        {
            ...
            try
            {
               ...
               + "WHERE  pm.[status] = '1' \n"
               + "AND pm.chkBillingModifier ='0' AND pm.Import is null \n"
               + "GROUP BY \n"
               + "       id, \n"
               + "       pm.name, \n"
               + "       pm.calculationValue, \n"
        ...
        {
            string sqlString = "SELECT consignmentNumber, totalAmount, chargedAmount, gst, DiscountID, DiscountApplied, DiscountGST FROM Consignment WHERE  \n"
        ...
        public DataTable SequenceCheck_Old(Cl_Variables clvar, string specialCondition)
        {
            string sql = "SELECT * \n"
                + "FROM mnp_riderCnSequence mzc \n"
                + "WHERE mzc.ZoneCode='" + clvar.Zone + "' AND \n"
