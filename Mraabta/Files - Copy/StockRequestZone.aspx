<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/BtsMasterPage.Master" CodeBehind="StockRequestZone.aspx.cs" Inherits="MRaabta.Files.StockRequestZone" %>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('body').on('click', '#btnModify', function () {
                ...
                for (let x of rows) {
                    var row = $(x);
                    var DetailId = parseInt(row.find('td:eq(2)').find('input[type=hidden]').val());
                    var typeId = parseInt(row.find('td:eq(0)').find('select').val());
                    var productId = parseInt(row.find('td:eq(1)').find('select').val());
                    productArr.push(productId);
                    var qty = parseInt(row.find('td:eq(2)').find('input[type=text]').val());
            ...
            $('body').on('change', '#dropdownTypeModal', function () {
                ...
                $(this).parent().parent().find('td:eq(1)').find('#dropdownProductModal').html(kk);
            });
        });
        var TypeProducts = [];
        ...
        var GetFilteredStockZone = (StartDate, EndDate) => {
            $.ajax({
                type: 'post',
                url: 'StockRequestZone.aspx/GetFilteredStockZone',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        var getRecordDetails = (Id) => {
            $.ajax({
                type: 'post',
                url: 'StockRequestZone.aspx/GetFilteredStockZonePer',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                data: JSON.stringify({ Id: Id }),
                success: (rs) => {
        ...
        var getRequestDetails = (id, isupdated, zone, name) => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockRequestZone.aspx/GetRequestsDetails',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                data: JSON.stringify({ id: id }),
                success: (rs) => {
        ...
        var save = (data) => {
            $.ajax({
                type: "POST",
                url: 'StockRequestZone.aspx/SaveStockDetails',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        var GetHOStock = () => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockRequestZone.aspx/GetHOStock',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    var data = rs.d;
        ...
        var GetZoneStock = () => {
            $.ajax({
                type: 'post',
                url: 'StockRequestZone.aspx/GetZoneStock',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    var data = rs.d;
