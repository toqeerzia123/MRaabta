...
{
    public partial class StockIssuanceRider : System.Web.UI.Page
    {
        ...
        public static List<CNIssuanceDropDownModel> GetRider()
        {
            ...
            try
            {
                ...
                var rs = con.Query<CNIssuanceDropDownModel>(@"select riderCode as [Value],(riderCode+' - ' + firstName +' - '+ isnull(hrs_Code,0) ) as [Text] from Riders where branchId = @branchCode and status = '1'", new
                {
        ...
        public static List<StockRequestModel> GetRequests(int? id)
        {
            ...
            try
            {
                ...
                if (id > 0)
                {
                    var rs = con.Query<StockRequestModel>(@"select sr.ID as [Id],sr.ECRiderCode as ZoneCode, r.riderCode+' - ' + r.firstName  as ZoneName, sr.TotalReqQty,FORMAT(sr.TotalReqQty,N'N0') as TotalReqQty_,  case when si.StockReqID is null then 0 else 1 end as [IsUpdated]    
                                                        from [MnP_CNIssue_StockRequest] sr
                                                        inner join Zones z on sr.ZoneCode= z.ZoneCode
                                                        inner join Riders r on r.riderCode = sr.ECRiderCode and r.branchId = @BranchCode
                                                        left join [MnP_CNIssue_StockIssuance] si on sr.ID = si.StockReqID
                                                        where sr.isActive = 1  
                                                              AND si.ID is null 
                                                                    AND sr.TotalReqQty > 0  AND sr.ECRiderCode=@RiderCode and sr.BranchCode = @BranchCode and sr.UserTypeID = '1005' ORDER BY sr.CreatedOn desc", new
                    {
                        @RiderCode = id,
                        @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString()
                    });
        ...
        {
            // return new StockIssuanceRiderRepo().GetRequestsDetails(id);
            try
            {
                con.Open();
                ...
                //    ZoneCode = (select ZoneCode from MnP_CNIssue_StockRequest where ID = @Id) and UserTypeID = 1004 and BranchID = (select BranchCode from MnP_CNIssue_StockRequest where ID = @Id) and BarcodeFrm between s.barcodeFrm and s.BarcodeTo) as xb 
                //    ) as QtyIssueFrom
        ...
        public static string SaveIssuanceDetails(StockIssuanceModel model)
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into MnP_CNIssue_StockIssuance (StockReqID,[Year],ZoneCode,BranchCode,ECRiderCode,TotalIssueQty,isActive,CreatedBy,CreatedOn,UserType) values(@StockReqID,@Year,@ZoneCode,@BranchCode,@ECRiderCode,@TotalIssueQty,1,@CreatedBy,GETDATE(),1005);;
                                                        SELECT SCOPE_IDENTITY();", new { @StockReqID = model.StockRequestId, @Year = DateTime.Now.Year, @ZoneCode = model.ZoneCode, @BranchCode = model.BranchCode, @ECRiderCode = model.ECRiderCode, @TotalIssueQty = model.StockIssuanceDetails.Sum(x => x.IssueQty), @CreatedBy = model.CreatedBy }, transaction: tran);
                    ...
                    foreach (var single in model.StockIssuanceDetails)
                    {
                        ...
                        insertStocktransit = con.Execute(@" insert into[Mnp_CNIssue_Stock] (StatusID,ProductID,ZoneCode,BranchID,ECRiderCode,BarcodeFrm,BarcodeTo,Qty,EntryDate,UserTypeID) values(1,@ProductID,@ZoneCode,@BranchCode,@ECRiderCode,@BarcodeFrm,@BarcodeTo,@IssueQty,GETDATE(),1005);
                                  ", new
                        {
                            StockIssuanceID = id,
                            StockReqDetailID = single.StockRequestDetailId,
