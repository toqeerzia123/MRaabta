...
{
    public partial class PettyCash_GLReport_MB : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (HttpContext.Current.Session["U_ID"].ToString() == "3144" || HttpContext.Current.Session["U_ID"].ToString() == "3136")
                {
                    Get_Zones();
                    Get_Companies();
                }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "view")
            {
                ...
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "OpenWindow", "window.open('PettyCash_GLprint_MB.aspx?from=" + frm_date + "&to=" + to_date + "&branch=" + branchcode + "&ec=" + ec + "&status=" + status + "&company=" + company + "&bname=" + branch_name + "');", true);
            }
        }
