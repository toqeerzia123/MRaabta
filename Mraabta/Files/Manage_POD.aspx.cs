    ...
    public partial class Manage_POD : System.Web.UI.Page
    {
        ...
        protected void gv_consignments_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                HiddenField hd_DRTV = (HiddenField)e.Row.FindControl("hd_DRTV");
                HiddenField hd_SupplierBranch = (HiddenField)e.Row.FindControl("hd_SupplierBranch");
                HiddenField hd_RunSheetBranch = (HiddenField)e.Row.FindControl("hd_RunSheetBranch");
                HiddenField hd_ActNo = (HiddenField)e.Row.FindControl("hd_ActNo");
                ...
                if (dr.Length > 0)
                {
                    ...
                    if (!(e.Row.Cells[1].Text.ToString().StartsWith("5") && e.Row.Cells[1].Text.ToString().Length == 15))
                    {
                        ...
                        {
                            ListItem RTS = dd_gReasons.Items.FindByValue("59"); // Return to  Origin
                            if (RTS != null)
                            {
                    ...
                    if (hd_reason.Value == "59")
                    {
                        ...
                        if (profileCode != "78") // Ediitng Writes
                        {
                            e.Row.Enabled = false;
                        }
                        else
                    ...
                    else if ((hd_RTO.Value.ToString().ToUpper() == "TRUE" || hd_RTO.Value.ToString() == "1") /*&& hd_origin.Value == HttpContext.Current.Session["BranchCode"].ToString()*/)
                    {
                        ...
                        // If Drtv =1 then supplier branch is equal to runsheetbranch  enable drtv reason or RTS bhe ana chaheye
                        // For Daraz
                        if (hd_ActNo.Equals("4J24"))
                        {
                            if (hd_DRTV.Value == "1")
                            {
                                if (hd_SupplierBranch == hd_RunSheetBranch)
                                {
                                    Drtv = "271";
                                }
                            }
                        ...
                        {
                            DataRow[] afterRTOReasons = reasons.Select("ID in ('59','64', '" + hd_reason.Value + "','" + Drtv + "')");
                        ...
                        {
                            // if Daraz Account then allow 59 id too
        ...
        {
            string sql = @"SELECT ROW_NUMBER() OVER(ORDER BY RC2.SortOrder DESC) SortOrder,  RC2.runsheetNumber RunsheetId,  c.consignmentNumber,
                        c.consignerAccountNo ActNo,  c.consignee, c.orgin
                        , (select case when rtntype = 1 then rtnbranch else c.orgin end from CreditClients where id = c.creditClientId) as rtnBranch
                        , c.destination, cdn.IsDRTV, sb.branchCode SupplierBranch, rb.branchCode RunSheetBranchCode,
                        ...
                        LEFT OUTER JOIN CODConsignmentDetail_New cdn ON cdn.consignmentNumber=RC2.consignmentNumber
                        INNER JOIN Branches b ON b.branchCode = c.orgin 
                        INNER JOIN Branches rb ON rb.branchCode = r.branchCode
        LEFT OUTER JOIN [codsqlserver.public.d056ebf0f0ac.database.windows.net, 3342].[MPEL_COD].dbo.tblNetworkguide codloc ON codloc.id=cdn.SupplierLocationId
                        LEFT OUTER JOIN Branches sb ON sb.sname = codloc.bookingbranch AND sb.status=1
                        ...
                        RC2.Reason, RC2.Comments, RC2.Receiver_CNIC, RC2.Relation, RC2.ModifiedOn, RC2.routeCode, RC2.GivenToRider,c.consignerCellNo, c.consignerAccountNo,cdn.IsDRTV, 
                        ri.riderCode + '-' + ri.firstName + ' ' + ri.lastName, c.cod, mp.prefix, c.isapproved, mco.RTO,c.consigneePhoneNo, c.consigner, c.ispayable,
                        rb.branchCode, sb.branchCode, cdn.IsDRTV
                        ORDER BY SortOrder";
            DataTable dt = new DataTable();
