...
{
    public partial class StockBranch_IMS : System.Web.UI.Page
    {
        ...
        public static string SaveStockDetails(StockRequestModel model)
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into [MnP_CNIssue_StockRequest] ([Year],ZoneCode, BranchCode, UserTypeID, TotalReqQty,isActive,CreatedBy,CreatedOn,LocationID) values(@Year,@zonecode,@BranchCode,1004,@TotalReqQty,1,@CreatedBy,GETDATE(),@LocationID);
                                                        SELECT SCOPE_IDENTITY();", new { @Year = DateTime.Now.Year, @ZoneCode = HttpContext.Current.Session["ZONECODE"].ToString(), @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(), @UserTypeID = model.UserTypeId, @TotalReqQty = model.StockRequestDetails.Sum(x => x.Qty), @CreatedBy = HttpContext.Current.Session["U_ID"].ToString(), @LocationID= model.StockRequestDetails.First().locationId }, transaction: tran);

                    foreach (var singleDetail in model.StockRequestDetails)
                    {
                        reqDetails = con.Execute(@"insert into [MnP_CNIssue_StockReqDetails] (StockReqID,ProductID, Qty,isActive,CreatedBy,CreatedOn,LocationID) values(@StockReqID,@ProductID, @Qty,1,@CreatedBy,GETDATE(),@LocationID);",
                        new
                        {
                            StockReqID = id,
