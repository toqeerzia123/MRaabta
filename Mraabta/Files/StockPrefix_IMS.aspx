<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/BtsMasterPage.Master" CodeBehind="StockPrefix_IMS.aspx.cs" Inherits="MRaabta.Files.StockPrefix_IMS" %>
                    ...
                    <table class="table table-bordered col-10">
                        <thead>
                            <tr style="font-size: 12px;">
                                ...
                                <th style="width: 10%">is Serialized</th>
                        ...
                        <div class="row ml-2">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input id="FilterPrefixTxt" maxlength="5" autopostback="true" onkeypress="return isNumberKey(event)" type="text" class="form-control" style="font-size: 12px;" />
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#btnSave').click(function () {
                ...
                for (let x of rows) {
                    ...
                    var type = row.find('td:eq(3)').find('.isSerialised').find('input[type="checkbox"]:checked').val() == 'on' ? 1 : 0;
                    console.log(type);
                    var PrefixLength = row.find('td:eq(3)').find('.isSerialised').find('input[type="checkbox"]:checked').val() == 'on' ?
                        parseInt(row.find('td:eq(4)').find('input[type=text]').val()) : 0;
                    var PrefixId = row.find('td:eq(3)').find('.isSerialised').find('input[type="checkbox"]:checked').val() == 'on' ?
                        parseInt(row.find('td:eq(5)').find('input[type=text]').val()) : 0;
                    productArr.push(ProductId);
                    if (type == 1 && (PrefixLength <= 0 || isNaN(PrefixLength))) {
                        alert('Please provide valid length');
                        return;
                    }
                    if (type == 1 && (PrefixId <= 0 || isNaN(PrefixId))) {
                ...
                $.ajax({
                    type: "POST",
                    url: 'StockPrefix_IMS.aspx/SavePrefixEntries',
            ...
            $('body').on('change', '.typeId', function () {
                ...
                var id = $(this).val();
                var kk = ProductFromType(id);
                const isserialised = isTypeSerialised(id);
                if (isserialised) {
                    $(this).parent().parent().find('td:eq(3)').find('.isSerialised').html(`<asp:CheckBox id="on" runat="server" Checked=true />
                    <span class="slider round"></span>`);
                    document.getElementById('ContentPlaceHolder1_on').disabled = true;
                    $(this).parent().parent().find('td:eq(4)').html(`<input maxlength="2" autopostback="true"  onkeypress="return isNumberKey(event)" type="text" class="form-control" style="font-size:12px;"/>`);
                    $(this).parent().parent().find('td:eq(5)').html(` <input maxlength="5" autopostback="true"  onkeypress="return isNumberKey(event)" type="text" class="form-control" style="font-size:12px;"/>`);


                }
                else {
                    $(this).parent().parent().find('td:eq(3)').find('.isSerialised').html(`<asp:CheckBox id="off" runat="server" Checked=false />
                    <span class="slider round"></span>`);
                    document.getElementById('ContentPlaceHolder1_off').disabled = true;

                    $(this).parent().parent().find('td:eq(4)').html(`<input maxlength="2" disabled="true" autopostback="true"  onkeypress="return isNumberKey(event)" type="text" class="form-control" style="font-size:12px;"/>`);
                    $(this).parent().parent().find('td:eq(5)').html(` <input maxlength="5" disabled="true" autopostback="true"  onkeypress="return isNumberKey(event)" type="text" class="form-control" style="font-size:12px;"/>`);
                }
        ...
        var getZones_01 = () => {
            $.ajax({
                async: false,
                type: "POST",
                url: 'StockPrefix_IMS.aspx/GetZones',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    var kk = '';
        ...
        var GetFilteredPrefix = (zone, Product, Prefix) => {
            $.ajax({
                type: 'post',
                url: 'StockPrefix_IMS.aspx/GetFilteredPrefix',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        var DisablePrefix = (Id, ZoneCode) => {
            $.ajax({
                type: 'post',
                url: 'StockPrefix_IMS.aspx/DisablePrefix',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        var getProducts = () => {
            $.ajax({
                type: "POST",
                url: 'StockPrefix_IMS.aspx/GetProducts',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    products = rs.d;
        ...
        var getTypes = () => {
            $.ajax({
                type: "POST",
                url: 'StockPrefix_IMS.aspx/GetTypeProducts',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    Types = rs.d;
        ...
        function ProductFromType(Id) {
            $.ajax({
                async: false,
                type: "POST",
                url: 'StockPrefix_IMS.aspx/GetProductsSingle',
                contentType: "application/json; charset=utf-8",
        ...
        var iss = '';
        function isTypeSerialised(TypeId) {
            $.ajax({
                async: false,
                type: "POST",
