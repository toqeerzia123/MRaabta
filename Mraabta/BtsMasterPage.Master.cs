...
namespace MRaabta
{
    public partial class BtsMasterPage : System.Web.UI.MasterPage
    {
        /*
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
               ...
               + "				ON  pd.ChildMenu_Id = c.Child_MenuId \n"
               + "	WHERE  m.Menu_id = '" + row["MENU_ID"].ToString() + "' \n"
               + "		   AND pd.Profile_Id = '" + row["Profile"].ToString() + "' and c.status='1'";
                        ...
                        if (pageGetURL.ToString().ToLower() != "btsdashoard.aspx")
                        {
                            ...
                            //Page_Usage_Log(Session["U_ID"].ToString(), pageGetURL, uri);
                            Page_Usage_Log(Session["U_ID"].ToString(), pageGetURL, uri, "MRaabtaMVC", "BtsMasterPage.master");
                        }
                        else
                        {
                            authorized = true;
        ...
        private void Page_Usage_Log(string U_ID, string pageGetURL, Uri uri, String systemType, String originName)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ip", ip2);
                cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
