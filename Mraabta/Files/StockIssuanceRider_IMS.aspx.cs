...
{
    public partial class StockIssuanceRider_IMS : System.Web.UI.Page
    {
        ...
        public static List<CNIssuanceDropDownModel> GetRider()
        {
            ...
            try
            {
                ...
                var rs = con.Query<CNIssuanceDropDownModel>(@"SELECT  r.riderCode as [Value],
                        (r.riderCode+' - ' + r.firstName +' - '+ isnull(r.hrs_Code,0) ) as [Text] from Riders r
                        INNER JOIN Routes rt ON  rt.routeCode=r.routeCode
                        where r.branchId =@branchCode and r.status = '1' AND rt.[status]=1", new
                {
        ...
        public static List<StockRequestModel> GetRequests(int? id, string startdate, string enddate)
        {
            ...
            try
            {
                ...
                {
                    var rs = con.Query<StockRequestModel>(@"select sr.ID as [Id],sr.ECRiderCode as ZoneCode, r.riderCode+' - ' + r.firstName  as ZoneName,CONVERT(VARCHAR(11),Sr.CreatedOn, 106) CreatedOn , sr.TotalReqQty,FORMAT(sr.TotalReqQty,N'N0') as TotalReqQty_,  case when si.StockReqID is null then 0 else 1 end as [IsUpdated]    
                                                        ...
                                                        where sr.isActive = 1 and r.status=1  
                                                              AND si.ID is null 
                                                                    AND sr.TotalReqQty > 0  AND sr.ECRiderCode=@RiderCode and sr.BranchCode = @BranchCode and sr.UserTypeID = '1005'  and cast(sr.createdon as date) between isnull(@startdate,sr.createdon) and isnull(@enddate,sr.createdon)  ORDER BY sr.CreatedOn desc", new
                    {
                        @RiderCode = id,
                        @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(),
                        @startdate = startdate,
                ...
                else
                {
                    var rs = con.Query<StockRequestModel>(@"select sr.ID as [Id],sr.ECRiderCode as ZoneCode,  r.riderCode+' - ' + r.firstName as ZoneName,CONVERT(VARCHAR(11),Sr.CreatedOn, 106) CreatedOn , sr.TotalReqQty, FORMAT(sr.TotalReqQty,N'N0') as TotalReqQty_, case when si.StockReqID is null then 0 else 1 end as [IsUpdated]    
                                                        ...
                                                        where sr.isActive = 1  and r.status=1  
                                                            AND si.ID is null 
                                                                AND sr.TotalReqQty > 0  and sr.BranchCode = @BranchCode and sr.UserTypeID = '1005'  and cast(sr.createdon as date) between isnull(@startdate,sr.createdon) and isnull(@enddate,sr.createdon)  ORDER BY sr.CreatedOn desc", new
                    {
                        @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(),
                        @startdate = startdate,
                        @enddate = enddate
  ...
  from (
        ...
        public static string SaveIssuanceDetails(StockIssuanceModel model)
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into MnP_CNIssue_StockIssuance (StockReqID,[Year],ZoneCode,BranchCode,ECRiderCode,TotalIssueQty,isActive,CreatedBy,CreatedOn,UserType,Remarks) values(@StockReqID,@Year,@ZoneCode,@BranchCode,@ECRiderCode,@TotalIssueQty,1,@CreatedBy,GETDATE(),1005,@Remarks);
