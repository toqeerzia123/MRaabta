...
{
    public partial class PettyCash_GLReport_hoa : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (HttpContext.Current.Session["U_ID"].ToString() == "3178" || HttpContext.Current.Session["U_ID"].ToString() == "4333")
                {
                    Get_Zones();
                    Get_Companies();
                    Get_FormType();
                    dd_sh.Items.Insert(0, new ListItem("Select Head Of Account", "0"));
                }
                else
                {
        ...
        public DataSet Mainhead()
        {
            ...
            if (dd_eh.SelectedValue.ToString() != "0")
            {
                form_type = " and pd.expense='" + dd_eh.SelectedValue.ToString() + "' ";
            }
            if (dd_sh.SelectedValue.ToString() != "0")
            {
                hoa = " and pd.[desc] ='" + dd_sh.SelectedValue.ToString() + "' ";
            }
            ...
            " left outer join pc_mainHead ft \n" +
            " on ft.code=pd.EXPENSE\n" +
            " left outer join pc_subHead hoa\n" +
            " on hoa.subcode=pd.[desc]\n" +
            "and hoa.headcode=ft.code \n" +
            " where \n" +
            ...
            " " + branch + " " + ec + " " + status + " " + form_type + " " + hoa + "  ";
            sqlString +=
            "   \n" +
        ...
        public void Get_FormType()
        {
            ...
            if (ds.Tables[0].Rows.Count != 0)
            {
                ...
                dd_eh.DataBind();
                dd_eh.Items.Insert(0, new ListItem("Select Form Type", "0"));
            }
        }
        public void Get_HOA()
        {
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "view")
            {
                ...
                string form_type = HttpUtility.UrlEncode(Encrypt(dd_eh.SelectedValue.ToString().Trim()));
                string hoa = HttpUtility.UrlEncode(Encrypt(dd_sh.SelectedValue.ToString()));
                //Redirect to the print view page
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "OpenWindow", "window.open('PettyCash_GLprint_hoa.aspx?from=" + frm_date + "&to=" + to_date + "&branch=" + branchcode + "&ec=" + ec + "&status=" + status + "&company=" + company + "&bname=" + branch_name + "&ft=" + form_type + "&hoa=" + hoa + "');", true);
            }
        }
