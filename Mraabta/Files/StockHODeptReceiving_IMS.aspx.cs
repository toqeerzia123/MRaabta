...
{
    public partial class StockHODeptReceiving_IMS : System.Web.UI.Page
    {
        ...
        static String U_ID = ""; static string DepartmentID = "";
        ...
        public static List<StockReceivingModel> GetIssuances()
        {
            ...
            try
            {
                ...
                {
                    condition = " and ml.DepartmentID in (" + DepartmentID + ")";
                }

                con.Open();
                var rs = con.Query<StockReceivingModel>(@"select pr.OracleCode, mu.Name Unit, SI.ID AS IssuanceID,sr.ID RequestID,ml.Department AS DepartmentName ,CONVERT(VARCHAR(11),Sr.CreatedOn, 106) CreatedOn ,sr.TotalReqQty, FORMAT(sr.TotalReqQty,N'N0') as TotalReqQty_ ,SI.TotalIssueQty, FORMAT(si.TotalIssueQty,N'N0') as TotalIssueQty_, sr.BranchCode
                                from MnP_CNIssue_StockRequest sr
                                inner join MnP_CNIssue_StockIssuance si on si.StockReqID = sr.ID
                                inner JOIN Department ml ON ml.departmentid=isnull(sr.ECRiderCode,ml.departmentid)
                                ...
                                where sr.UserTypeID='1008' and src.id is null and sr.BranchCode=@BranchCode and sr.ZoneCode = '28' order by sr.CreatedOn", new { @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(), @Zone = HttpContext.Current.Session["ZONECODE"].ToString() });
                con.Close();
                return rs.ToList();
            }
        ...
        public static List<StockReceivingDetailModel> GetIssuancesDetails(int ID)
        {
            ...
            try
            {
                ...
                inner join MnP_CNIssue_StockIssuanceDetail id on id.StockReqDetailID = rd.ID
				inner join MnP_CNIssue_StockIssuance s on s.ID=id.StockIssuanceID
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
	             ...
	             sid where sid.StockIssuanceID=@IssuanceID", new { @IssuanceID = ID }, transaction: tran);
                    ...
                    var id = con.QueryFirstOrDefault<int>(@"insert into MnP_CNIssue_StockRecieving(StockIssuanceID,ECRiderCode,[Year],ZoneCode,[BranchCode],UserTypeID,TotalRecieveQty,isActive,CreatedBy,CreatedOn) 
	                                                    values(@StockIssueID,@DepartmentID, @Year,'28',@BranchCode,'1008',@TotalReceiveQty,1,@CreatedBy,GETDATE());;
                    ...
                    foreach (var single in detailModel)
                    {
                        rs = con.Execute(@"insert into MnP_CNIssue_StockRecievingDetail(StockRecievingID,StockIssuanceDetailID,RecievingQty,BarcodeFrm,BarcodeTo,CreatedBy,CreatedOn,isActive) values(@StockRecievingID,@StockIssuanceDetailID,@RecievingQty,@BarcodeFrm,@BarcodeTo,@CreatedBy,GETDATE(),1);"
                         , new
                         {
                            ...
                            update = con.Execute(@"update MnP_CNIssue_Stock set StatusID = 1, ZoneCode = '28'
                                               WHERE barcodeFrm = @BarcodeFrm and BarcodeTo = @BarcodeTo and ProductID=(
											select ProductID from MnP_CNIssue_StockReqDetails
											where id =(select stockreqdetailid from MnP_CNIssue_StockIssuanceDetail where id="+single.stockIssuanceDetailID+@")) and Qty=@RecvQty and statusID = 5"
                          , new
                          {
                              BarcodeFrm = single.BarcodeFrom,
                              BarcodeTo = single.BarcodeTo,
                              RecvQty = single.IssueQty
                          }, transaction: tran);
        ...
        public static List<StockModel> GetMyStock()
        {
            ...
            {
                con.Open();
                var rs = con.Query<StockModel>(@"select pr.OracleCode,productID,pr.ProductName,t.TypeName,d.Department DepartmentName, sum(qty) as Qty, FORMAT(sum(qty),N'N0') as Qty_  from (
                                            select ProductID, zonecode, ECRiderCode DepartmentID, case when StatusID = 2 then qty * (-1) 
                                            when StatusID = 1 then qty else qty * 0 end as qty from Mnp_CNIssue_Stock
                                            where UserTypeID = '1008'
                                            ) as xb
                                            ...
                                            inner join Department d on cast(d.DepartmentID as varchar)=xb.DepartmentID
                                            group by pr.OracleCode,ProductID, xb.ZoneCode, xb.DepartmentID,ProductName,TypeName,Department
                                            having xb.DepartmentID = @DepartmentID", new
                {
                    @DepartmentID = HttpContext.Current.Session["DepartmentID"].ToString()
                });
                con.Close();
