...
{
    public partial class StockBranchIssuance_IMS : System.Web.UI.Page
    {
        ...
        public static List<StockRequestModel> GetRequests(int? id, string startdate, string enddate)
        {
            ...
            try
            {
                ...
                {
                    var rs = con.Query<StockRequestModel>(@"select pr.OracleCode,sr.ID as [Id], sr.BranchCode,b.name as BranchName ,sr.LocationID, isnull(ml.name,'-') AS LocationName,b.branchCode, 
                                                        ...
                                                        where sr.isActive = 1  AND si.ID is null AND sr.TotalReqQty > 0 and sr.BranchCode=@BranchCode and sr.UserTypeID='1004'   and cast(sr.createdon as date) between isnull(@startdate,sr.createdon) and isnull(@enddate,sr.createdon) ", new { @BranchCode = id, @startdate = startdate, @enddate = enddate });
                    return rs.ToList();
                }
                else
                {
                    var rs = con.Query<StockRequestModel>(@"select mu.name Unit, pr.OracleCode, sr.ID as [Id],z.name as BranchName,sr.LocationID, isnull(ml.name,'-') AS LocationName,sr.branchCode,sr.zoneCode, sr.TotalReqQty,FORMAT(sr.TotalReqQty,N'N0') TotalReqQty_,CONVERT(VARCHAR(11),Sr.CreatedOn, 106) CreatedOn, case when si.StockReqID is null then 0 else 1 end as [IsUpdated]  
                    from [MnP_CNIssue_StockRequest] sr
                    ...
                    left join [MnP_CNIssue_StockIssuance] si on sr.ID = si.StockReqID
                    LEFT JOIN MNP_Locations ml ON ml.id= sr.LocationID AND ml.[status]=1
                    where sr.isActive = 1  AND si.ID is null AND sr.TotalReqQty > 0 and sr.ZoneCode=@Zone and sr.UserTypeID=1004  and cast(sr.createdon as date) between isnull(@startdate,sr.createdon) and isnull(@enddate,sr.createdon) ", new { @Zone = HttpContext.Current.Session["ZONECODE"].ToString(), @startdate = startdate, @enddate = enddate });
                    return rs.ToList();
                }
            }
            catch (Exception ex)
        ...
        public static string SaveIssuanceDetails(StockIssuanceModel model)
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into MnP_CNIssue_StockIssuance (StockReqID,[Year],ZoneCode,BranchCode,TotalIssueQty,isActive,CreatedBy,CreatedOn,UserType,FOCNumber,Remarks) values(@StockReqID,@Year,@ZoneCode,@BranchCode,@TotalIssueQty,1,@CreatedBy,GETDATE(),1004, @FOCNumber,@Remarks);
                                                        SELECT SCOPE_IDENTITY();", new { @StockReqID = model.StockRequestId, @Year = model.Year, @ZoneCode = HttpContext.Current.Session["ZONECODE"].ToString(), @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(), @TotalIssueQty = model.StockIssuanceDetails.Sum(x => x.IssueQty), @CreatedBy = HttpContext.Current.Session["U_ID"].ToString(), @FOCNumber = model.FOCNumber != "" ? model.FOCNumber : "", @Remarks = model.Remarks != "" ? model.Remarks : "" }, transaction: tran);
