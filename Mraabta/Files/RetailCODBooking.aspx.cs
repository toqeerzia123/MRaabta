...
{
    public partial class RetailCODBooking : System.Web.UI.Page
    {
        ...
        String ServiceType = "";
        string CODType = "", AccountNo = "", ConsignerCellNo = "", phone, nic;
        DataTable dt_discount;
        string CustomerClientIDForRetailCOD = "";
        string limitover = "";

        protected void Page_Load(object sender, EventArgs e)
        {
            //if (HttpContext.Current.Session["U_ID"].ToString() != "2974")
            //{
        ...
        protected void cb_Account1_CheckedChanged(object sender, EventArgs e)
        {
            txt_ConsignerCellNo_TextChanged1(sender, e);
        }
        ...
        protected void rd_AddPriceModifier_Click(object sender, EventArgs e)
        {
            ...
            DataTable seq = SequenceCheck(clvar, " c.AccountNo = '" + txt_AccNo.Text.Trim() + "'");
            if (seq.Rows.Count <= 0)
            {
            ...
            double cn = double.Parse(seq.Rows[0]["LastCNIssued"].ToString());

            clvar.consignmentNo = (cn + 1).ToString();
            txt_ConNo.Text = (cn + 1).ToString();
            ...
            if (dr.Length == 0)
            {
                ...
                if (txt_Description2.Text != "Extra Charges")
                {
                    ...
                    dr_["ConsignmentNumber"] = Int64.Parse(txt_ConNo.Text.Trim());
                    dr_["CreatedBy"] = "TEST";
        ...
        protected void btn_SaveConsignment1_Click(object sender, EventArgs e)
        {
            ...
            //        {
            //            Int64 start = Int64.Parse(dr["SequenceStart"].ToString());
            //            Int64 end = Int64.Parse(dr["EndSequence"].ToString());
            //            Int64 cn = Int64.Parse(txt_ConNo.Text.Trim());
            //            if (cn >= start && cn <= end && dr["PRODUCTS"].ToString().ToUpper() == cb_ServiceType.SelectedItem.Text.ToUpper())
            //            {
            //                valid = true;
            //                break;
            //            }
            ...
            if (txt_CodAmount.Text.Trim() == "" || float.Parse(txt_CodAmount.Text) == 0)
            {
                Alert("COD Amount can't be Zero Amount.");
                return;
            }

            if (int.Parse(txt_CodAmount.Text) > 40000)
            {
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('You have enter exceed amount. Max Amount is 40,000')", true);
                return;
            }
            ...
            */
            ...
            Tuple<bool, string> AccNoCheckDefault = AccountNumberDefaultCheck();
            if (AccNoCheck.Item1 == false)
            {
                Alert(AccNoCheck.Item2.ToString());
                btn_SaveConsignment0.Enabled = false;
            ...
            try
            {
                ...
                clvar.AccountNo = "99";// txt_AccNo.Text;
                clvar.RiderCode = txt_RiderCode.Text;
                ...
                cb_originExpresscenter.SelectedValue = Session["ExpressCenter"].ToString();
                ...
                DataTable ClientID = CustomerClientID(clvar);

                CustomerClientIDForRetailCOD = ClientID.Rows[0]["ID"].ToString();
        ...
        protected void btn_SaveConsignment_Click(object sender, EventArgs e)
        {
            ...
            //this.btn_SaveConsignment.Enabled = false;
            lbl_Error.Text = "";
            try
            {
                ...
                //DataTable seq = SequenceCheck(clvar, " AND C.CreditClientId = (SELECT CC.ID FROM CreditClients cc WHERE cc.accountNo = '" + clvar.consignerAccountNo + "')");
                DataTable seq = SequenceCheck(clvar, " c.AccountNo = '" + txt_AccNo.Text.Trim() + "'");
                if (seq.Rows.Count <= 0)
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('This CN Number cannot be used for selected Service')", true);
                ...
                string cod_location = "SELECT * FROM COD_CustomerLocations WHERE accountNo = '" + clvar.AccountNo + "'";
                SqlConnection cons = new SqlConnection(clvar.Strcon());
                try
                {
                    cons.Open();
                    SqlDataAdapter sda = new SqlDataAdapter(cod_location, cons);
                    sda.Fill(loc_dt);
                }
                catch (Exception ex)
                { }
                ...
                {
                    clvar.LoadingID = loc_dt.Rows[0]["locationID"].ToString();
                }
                ...
                foreach (RepeaterItem item in RadGrid1.Items)
                {
                    ...
                    dtcRow["consignmentNumber"] = clvar.consignmentNo;
                ...
                */
                ...
                if (error == "")
                {
                    ...
                    if (dd_discount.SelectedValue == "0")
                    {
                        ...
                        string ecnryptedConsignment = EnryptString(clvar.consignmentNo);
                    ...
                    if (dd_discount.SelectedValue == "0")
                    {
                        if (dd_discount.SelectedValue == "0")
                        {
                            ...
                            string u_qty = "UPDATE CONSIGNMENT SET chargedAmount = '" + clvar.ChargeAmount + "' WHERE CONSIGNMENTNUMBER = '" + clvar.consignmentNo + "'";
                            try
                            {
                                sqlcon.Open();
                                SqlCommand sqlcmd = new SqlCommand(u_qty, sqlcon);
        ...
        public string Add_Consignment(Cl_Variables obj, float denseWeight, string remarks)
        {
            ...
            {
                sqlcon.Open();
                SqlCommand sqlcmd = new SqlCommand("MNP_InsertConsignment_RetailCOD", sqlcon);
                ...
                sqlcmd.Parameters.AddWithValue("@locationID", obj.LoadingID);
        ...
        public DataTable SequenceCheck(Cl_Variables clvar, string specialCondition)
        {
            ...
            //string sql = @"SELECT * FROM MNP_Retail_COD_Info C 
            //           WHERE '" + clvar.consignmentNo + @"' BETWEEN c.SequenceStart AND c.SequenceEnd
            //           " + specialCondition + @"
            //        ";

            string sql = @"SELECT * FROM MNP_Retail_COD_Info C 
                       WHERE " + specialCondition;
        ...
        public Tuple<bool, string> AccountNumberDefaultCheck()
        {
            Tuple<bool, string> errorTuple;
            ...
            if (txt_AccNo.Text != "")
            {
                if (txt_AccNo.Text != "")
                {
                    clvar.Branch = cb_Origin.SelectedValue;
                    clvar.AccountNo = txt_AccNo.Text;
                    DataSet ds = CustomerInformationForRetailCOD(clvar);

                    if (ds.Tables.Count != 0)
                    {
                        DataRow[] dr = ds.Tables[0].Select("AccountNo = '99'");
                        if (dr.Count() == 0)
                        {
                            errorTuple = new Tuple<bool, string>(false, "Invalid Account Number");
                            return errorTuple;
                        ...
                        if (ds.Tables[0].Rows.Count != 0)
                        {
                            hd_CreditClientIDForRetailCOD.Value = dr[0]["id"].ToString();
                            ...
                            if (dr[0]["IsCOD"].ToString() == "True")
                            {
                                ...
                                //Now Populating Product Type of COD
                                CustomerClientIDForRetailCOD = dr[0]["id"].ToString();
                            }
                            else
                            {
                                cb_COD.Checked = false;
            ...
            if (accountNumber.Trim() == "0")
            {
                ...
                if (seqTbl != null)
                {
                    if (seqTbl.Rows.Count > 0)
                    {
                        ...
                        if (item != null)
                        {
                            cb_originExpresscenter.SelectedValue = Session["ExpressCenter"].ToString(); //seqTbl.Rows[0]["ExpressCenter"].ToString();
        ...
        {
            string sqlString__OLD = "SELECT consignmentNumber, totalAmount, chargedAmount, gst, DiscountID, DiscountApplied, DiscountGST, consignerAccountNo \n"
                               + "FROM Consignment WHERE  \n"
                               + "DiscountID = '" + txt_discountId.Text + "' \n"
                               + "AND originExpressCenter = '" + HttpContext.Current.Session["ExpressCenter"].ToString() + "'  \n"
                               + "AND orgin = '" + HttpContext.Current.Session["BRANCHCODE"].ToString() + "' \n"
                               + "AND CAST(bookingdate AS date) = cast(GETDATE() AS date) \n"
                               + clvar.VoucherNo;

            string sqlString__OLD2 = @"SELECT c.consignmentNumber, c.totalAmount, c.chargedAmount, c.gst, c.DiscountID, c.DiscountApplied, c.DiscountGST, 
                              ...
                              c.consigner, c.shipperAddress, c.consignerCellNo, c.consignerCNICNo
            ...
            string sqlString = @"SELECT c.consignmentNumber, c.totalAmount, c.chargedAmount, c.gst, c.DiscountID, c.DiscountApplied, c.DiscountGST, 
                               c.consigner, c.shipperAddress, c.consignerCellNo, c.consignerCNICNo, SUM(ISNULL(cm.modifiedCalculationValue,'0')) ModifierValue,
                               isnull(cm.calculatedValue,0) calculatedValue, isnull(cm.calculatedGST,0) calculatedGST, consignerAccountNo
                               FROM 
                               Consignment c
                               LEFT JOIN ConsignmentModifier cm ON cm.consignmentNumber = c.consignmentNumber
                               WHERE  
                              ...
                              + clvar.VoucherNo +
                              @"GROUP BY c.consignmentNumber, c.totalAmount, c.chargedAmount, c.gst, c.DiscountID, c.DiscountApplied, c.DiscountGST, 
                              c.consigner, c.shipperAddress, c.consignerCellNo, c.consignerCNICNo, cm.calculatedValue, cm.calculatedGST, consignerAccountNo
                              Order by c.consignmentNumber DESC";
        ...
        {
            string contactNo = Txt_ConsignerCNIC.Text;
            double sAsD = double.Parse(contactNo);
            Txt_ConsignerCNIC.Text = string.Format("{0:#-#######-#}", sAsD).ToString();
        }
        protected void txt_ConsignerCellNo_TextChanged1(object sender, EventArgs e)
        {
            ...
            try
            {
                string phoneQuery_ = "", accountNoQuery = "", nicQuery = "";
                lbl_Error.Text = "";

                if (txt_discountId.Text == "")
                {
                    if (txt_ConsignerCellNo.Text != "")
                    {
                        string contactNo = txt_ConsignerCellNo.Text;
                        double sAsD = double.Parse(contactNo);
                        phone = "0" + string.Format("{0:#-#######}", sAsD).ToString();

                        phoneQuery_ = "AND cc.phoneNo = '" + phone + "' \n";
                    }

                    if (Txt_ConsignerCNIC.Text != "")
                    {
                        string nic = Txt_ConsignerCNIC.Text;
                        double sAsD = double.Parse(nic);
                        nic = string.Format("{0:#-#######-#}", sAsD).ToString();

                        nicQuery = " AND cc.NTNNO = '" + nic + "' \n";
                    }
                }

                if (txt_AccNo.Text != "")
                {
                    accountNoQuery = " AND cc.accountNo = '" + txt_AccNo.Text + "' \n";
                }

                string query = "SELECT MR.LastCNIssued, CC.* FROM CreditClients CC \n" +
                               "INNER JOIN MNP_Retail_COD_Info MR ON CC.ID = MR.CreditClientId \n" +
                               "WHERE cc.CODType = '3' AND cc.isActive = '1' AND ISNULL(cc.IsCOD,'0') = '1' \n" +
                               "AND cc.branchCode = '" + HttpContext.Current.Session["BRANCHCODE"].ToString() + "' " + phoneQuery_ + accountNoQuery + nicQuery;
                ...
                if (Ds_1.Tables[0].Rows.Count > 0)
                {
                    txt_ConsignerCellNo.Text = Ds_1.Tables[0].Rows[0]["PHONENO"].ToString();
                    txt_Consigner.Text = Ds_1.Tables[0].Rows[0]["Name"].ToString();
                    txt_ShipperAddress.Text = Ds_1.Tables[0].Rows[0]["Address"].ToString();
                    Txt_ConsignerCNIC.Text = Ds_1.Tables[0].Rows[0]["ntnNo"].ToString();
                    txt_AccNo.Text = Ds_1.Tables[0].Rows[0]["accountNo"].ToString();
                    txt_ConNo.Text = Ds_1.Tables[0].Rows[0]["LastCNIssued"].ToString();

                    //txt_ConsignerCellNo.Enabled = false; 
                    txt_ConNo.Enabled = false;
                    txt_Consigner.Enabled = false;
                    txt_ShipperAddress.Enabled = false;
                    Txt_ConsignerCNIC.Enabled = false;
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Invalid Account Detail.')", true);
                    lbl_Error.Text = "Invalid Account No.";
                    return;
                }
            }
            catch (Exception Err)
        ...
        protected void discount_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            clvar.InvoiceNo = "AND DiscountID = '" + DiscountID.ToString() + "' AND c.DiscountApplied is null  ";
        ...
        protected void discountId_Fixed_TextChanged____OLD(object sender, EventArgs e)
        {
            ...
            if (dt_discount.Rows.Count > 0)
            {
                ...
                if (txt_discountId.Text != "")
                {
                    ...
                    clvar.VoucherNo = "AND DiscountApplied IS null \n" +
                                            ...
                                            "AND ISNULL(STATUS,'0') = '9' \n" +
                                            "AND ISNULL(isApproved,'0') = '0' \n" +
                                            "AND ISNULL(isPriceComputed,'0') = '0' \n";
                    ...
                    {
                        txt_AccNo.Text = discn_dt.Rows[0]["consignerAccountNo"].ToString();
                        txt_AccNo.Enabled = false;
                        txt_ConsignerCellNo_TextChanged1(sender, e);
        ...
        protected void discountId_Fixed_TextChanged(object sender, EventArgs e)
        {
            if (dd_discount.SelectedValue != "0" || txt_discountId.Text != "")
            {
                ...
                {
                    clvar.InvoiceNo = "AND c.DiscountID = '" + txt_discountId.Text.Trim() + "' AND DiscountApplied is null";
                    ...
                    if (txt_discountId.Text != "")
                    {
                        ...
                        if (discn_dt.Rows.Count > 0)
                        {
                            for (int i = 0; i < discn_dt.Rows.Count; i++)
                            {
                                ...
                                txt_AccNo.Text = discn_dt.Rows[i]["consignerAccountNo"].ToString();
                                txt_AccNo.Enabled = false;
                            }
        ...
        public string Add_Consignment_Validation(Cl_Variables obj)
        {
            ...
            try
            {
                ...
                sqlcmd.Parameters.AddWithValue("@pieces", obj.pieces);
                sqlcmd.Parameters.AddWithValue("@serviceTypeName", obj.ServiceTypeName);
                sqlcmd.Parameters.AddWithValue("@creditClientId", CustomerClientIDForRetailCOD); //-----------
                sqlcmd.Parameters.AddWithValue("@consignmentTypeId", obj.Con_Type);
                sqlcmd.Parameters.AddWithValue("@weight", obj.Weight);
                sqlcmd.Parameters.AddWithValue("@weightUnit", obj.Unit);
                sqlcmd.Parameters.AddWithValue("@discount", obj.Discount);
                ...
                sqlcmd.Parameters.AddWithValue("@consignerCNICNo", obj.ConsignerCNIC);
                //sqlcmd.Parameters.AddWithValue("@consignerAccountNo", obj.consignerAccountNo); // ----------------------
                sqlcmd.Parameters.AddWithValue("@consignerAccountNo", "99"); // ----------------------
                sqlcmd.Parameters.AddWithValue("@consignerEmail", obj.consignerEmail);
                sqlcmd.Parameters.AddWithValue("@docIsHomeDelivery", obj.docIsHomeDelivery);
                sqlcmd.Parameters.AddWithValue("@cutOffTime", obj.cutOffTime);
                sqlcmd.Parameters.AddWithValue("@destinationCountryCode", obj.destinationCountryCode);
        ...
        public DataSet ServiceTypeName()
        {
            ...
            try
            {
                string query = "SELECT c.ServiceTypeName FROM MnP_Customer_ServiceMap c WHERE c.CreditClientID = '472035' and c.status=1";
        ...
        public DataSet CustomerInformationForRetailCOD(Cl_Variables clvar)
        {
            DataSet Ds_1 = new DataSet();
        ...
        public DataTable CustomerClientID(Cl_Variables clvar)
        {
            DataTable Ds_1 = new DataTable();

            try
            {
                string query = "SELECT cc.id FROM CreditClients AS cc WHERE cc.accountNo = '99' AND cc.branchCode = '" + clvar.origin + "'";
