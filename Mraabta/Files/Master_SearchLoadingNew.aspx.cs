...
{
    public partial class Master_SearchLoadingNew : System.Web.UI.Page
    {
        ...
        protected void Btn_Save_Click(object sender, EventArgs e)
        {
            if (txt_loading.Text.Trim() != "" || txt_date.Text.Trim() != "" || txt_sealNo.Text.Trim() != "" || dd_destination.SelectedValue != "0" || dd_transportType.SelectedValue != "0")
            {
                ...
                if (txt_date.Text.Trim() != "")
                {
                    // clvar.Check_Condition += " AND CAST(l.date as DATE) = '" + DateTime.Parse(txt_date.Text).ToString("yyyy-MM-dd") + "'\n";
                }
                if (txt_loading.Text != "")
                {
        ...
        public DataSet Get_SearchLoading(Variable clvar)
        {
            ...
            try
            {
                string sql = " select l.id,lu.AttributeDesc TransportType, \n" +
                             ...
                             "        (SUM(CAST(ISNULL(lb.BagWeight, 0) as float)) + SUM(CAST(ISNULL(lc.cnWeight, 0) as float))) TotalWeight, Case when isairport ='1' then 'Yes' else 'No' end AtAirport \n" +
                             "   from mnp_Loading l \n" +
                             ...
                             " WHERE  CAST(l.date as DATE) = '" + DateTime.Parse(txt_date.Text).ToString("yyyy-MM-dd") + "' " + clvar.Check_Condition + "\n" +
