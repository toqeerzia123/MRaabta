...
{
    public partial class StockInventory_ims : System.Web.UI.Page
    {
        ...
        public static List<StockProductModel> GetStockItems(string OracleCode)
        {
            ...
            {
                constatic.Open();
                var rs = constatic.Query<StockProductModel>(@" select OracleCode, p.TypeId,p.ID ProductId, ProductName,t.TypeName,isnull(p.isSerialised,0) isSerialised, u.Name Unit from MnP_CNIssue_Product p
        ...
        public static List<StockModel> GetHOInventory()
        {
            try
            {
                ...
                select pr.OracleCode, pr.ProductName,t.TypeName,z.name as ZoneCode, sum(qty) as Qty, FORMAT(sum(qty),N'N0') as Qty_  from (
        ...
        public static List<CNIssuanceDropDownModel> GetZones()
        {
            ...
            {
                constatic.Open();
                var rs = constatic.Query<CNIssuanceDropDownModel>(@"select [zoneCode] as Value,[Name] as Text from Zones where zonecode='28'"); // region is not null order by name asc;");
                constatic.Close();
                return rs.ToList();
            }
            catch (SqlException ex)
        ...
        public static string SaveStockDetails(StockRequestModel model)
        {
            ...
            using (var tran = constatic.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        string query = $@"insert into[MnP_CNIssue_Inventory] (IsActive, ProductID, ZoneCode, BarcodeFrm, BarcodeTo, Qty, CreatedOn, CreatedBy, Year,PONo,DCNo) 
values(1,{item.ProductId},{item.ZoneCode},'{item.BarcodeFrm}','{item.BarcodeTo}',{item.Qty},GETDATE(),'{HttpContext.Current.Session["U_ID"].ToString()}','{DateTime.Now.Year}','{item.PONo}','{item.DCNo}'); select scope_identity();";

                        inventoryInsert = Convert.ToString(constatic.ExecuteScalar(query, transaction: tran));

                        stockInsert = constatic.Execute(@"insert into[Mnp_CNIssue_Stock] (StatusID,InvID,ProductID,ZoneCode,BarcodeFrm,BarcodeTo,UserTypeID,Qty,EntryDate, Year) values(1,@InvID,@ProductID,@ZoneCode,@BarcodeFrm,@BarcodeTo,1002,@Qty,GETDATE(),@year_);"
                            , new
                            {
                                InvID = inventoryInsert,
...
//                    // xb.barcodefrom +(xb.Qty - xb.QtyIssueFrom ) is replaced to xb.barcodefrom  -- code added by komal on 25 Feb 2022

//                    var rs = constatic.Query<StockRequestDetailModel>(@"select i.ID as Inv_ID,xb.ID as stockID,xb.ProductID,pr.ProductName,xb.ZoneCode,z.name ,t.TypeName,xb.BarcodeFrom as BarcodeFrom, xb.BarcodeTo,xb.QtyIssueFrom as Qty , FORMAT(xb.QtyIssueFrom,N'N0') as Qty_,
...
//                    rs = constatic.Query<StockRequestDetailModel>(@"select i.ID as Inv_ID,s.ID as stockID,s.ProductID,pr.ProductName,s.ZoneCode,z.name ,t.TypeName,s.BarcodeFrm as BarcodeFrom, s.BarcodeTo,
//s.Qty as Qty, FORMAT(s.Qty,N'N0') as Qty_,
//                pr.TypeId--, case when s.Qty - s.QtyIssueFrom > 0 then 'true' else 'false' end IsUpdated
//                from Mnp_CNIssue_Stock s             
//                inner join MnP_CNIssue_Inventory i on s.ProductID = i.ProductID and s.ZoneCode = i.ZoneCode and s.BarcodeFrm = i.BarcodeFrm  and s.BarcodeTo = i.BarcodeTo AND i.ID=s.invid
//                inner join [MnP_CNIssue_Product] pr on s.ProductID = pr.ID
//                inner join [MnP_CNIssue_Type] t on t.id = pr.TypeId
//                inner join Zones z on s.zonecode = z.zoneCode 
//                where s.UserTypeID =  1002 
//                and statusID = 1 and EntryDate between @startt+' 00:00:00.000' and @endd+' 23:59:59.999' and s.ZoneCode= @zone
//                order by i.ID,s.ID", new { @startt = StartDate, @endd = EndDate, @zone = zone });
//                    constatic.Close();
...
//                {
                ...
                from Mnp_CNIssue_Stock s 
                where s.UserTypeID =  1002 
                and statusID = 1 and  cast(EntryDate as date) between @startt and @endd ) as xb 
                inner join MnP_CNIssue_Inventory i on xb.ProductID = i.ProductID and xb.ZoneCode = i.ZoneCode and xb.BarcodeFrom = i.BarcodeFrm 
                and xb.BarcodeTo = i.BarcodeTo 
                inner join [MnP_CNIssue_Product] pr on xb.ProductID = pr.ID
                inner join [MnP_CNIssue_Type] t on t.id = pr.TypeId
                ...
                order by i.ID,xb.ID", new { @startt = StartDate, @endd = EndDate });
                    ...
                    rs = constatic.Query<StockRequestDetailModel>(@"select i.ID as Inv_ID,s.ID as stockID,pr.OracleCode,s.ProductID,pr.ProductName,s.ZoneCode,z.name ,t.TypeName,s.BarcodeFrm as BarcodeFrom, s.BarcodeTo,
s.Qty as Qty, FORMAT(s.Qty,N'N0') as Qty_,mu.Name Unit,
                pr.TypeId--, case when s.Qty - s.QtyIssueFrom > 0 then 'true' else 'false' end IsUpdated
                from Mnp_CNIssue_Stock s             
                inner join MnP_CNIssue_Inventory i on s.ProductID = i.ProductID and s.ZoneCode = i.ZoneCode and s.BarcodeFrm = i.BarcodeFrm  and s.BarcodeTo = i.BarcodeTo AND i.ID=s.invid
                inner join [MnP_CNIssue_Product] pr on s.ProductID = pr.ID
                ...
                where s.UserTypeID =  1002 and z.zonecode='28'
                and statusID = 1 and cast(EntryDate as date) between @startt and @endd
                order by i.ID,s.ID", new { @startt = StartDate, @endd = EndDate });
                    constatic.Close();
