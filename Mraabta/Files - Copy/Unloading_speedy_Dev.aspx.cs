...
{
    public partial class Unloading_speedy_Dev : System.Web.UI.Page
    {
        ...
        protected void txt_loadingID_TextChanged(object sender, EventArgs e)
        {
            if (txt_vid.Text.Trim(' ') != "")
            {
                ...
                DataTable tempdt = new DataTable();//con.GetLoadingBags(clvar);

                DataTable tempdtC = new DataTable();// con.GetConsignmentForUnload(clvar);

                DataTable dtUnload = new DataTable();
                if (ViewState["dtUnload"] != null)
                {
                    dtUnload = (DataTable)ViewState["dtUnload"];
                }
                else
                {
                    dtUnload.Columns.Add("loadingID");
                    dtUnload.Columns.Add("Route");
                    dtUnload.Columns.Add("Origin");
                    dtUnload.Columns.Add("Bag");
                    dtUnload.Columns.Add("CN");
                    dtUnload.Columns.Add("Weight", typeof(Decimal));
                }
                if (dtUnload.Select("loadingID = '" + txt_vid.Text.ToString() + "'").Count() < 1)
                {
                    if (detail != null)
                    {
                        ...
                        if (detail.Rows.Count > 0)
                        {
                            ...
                            if (detail.Rows.Count != 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        dt = ViewState["bags"] as DataTable;
                                        tempdt = GetLoadingBags(clvar);
                                        dt.Merge(tempdt);
                                        ViewState["bags"] = dt;
                                    }
                                    else
                                    {
                                        tempdt = GetLoadingBags(clvar);
                                        ViewState["bags"] = tempdt;
                                        dt = tempdt;
                                    }

                                    if (ViewState["cns"] != null)
                                    {
                                        dtC = ViewState["cns"] as DataTable;
                                        tempdtC = GetConsignmentForUnload(clvar);
                                        dtC.Merge(tempdtC);
                                        ViewState["cns"] = dtC;
                                    }
                                    else
                                    ...
                                    {
                                        Decimal wt = 0;
                                        //tempdt.Compute("sum(TotalWeight)", string.Empty);   // column datatype conversion problem from string to int
                                        foreach (DataRow row in tempdt.Rows) wt += Convert.ToDecimal(row["TotalWeight"]);
                                        foreach (DataRow row in tempdtC.Rows) wt += Convert.ToDecimal(row["weight"]);

                                        DataRow drUnload = dtUnload.NewRow();
                                        drUnload["loadingID"] = txt_vid.Text;
                                        drUnload["Route"] = detail.Rows[0]["RouteName"].ToString();
                                        drUnload["Origin"] = detail.Rows[0]["OriginBranch"].ToString();
                                        drUnload["Bag"] = tempdt.Rows.Count;
                                        drUnload["CN"] = tempdtC.Rows.Count;
                                        drUnload["Weight"] = wt;


                                        dtUnload.Rows.Add(drUnload);
                                    }

                                    ViewState["dtUnload"] = dtUnload;

                                    gvLoading.DataSource = dtUnload;
                                    gvLoading.DataBind();

                                    txt_totalLoadWeight.Text = dtUnload.Compute("sum(Weight)", string.Empty).ToString();
                                }
                                if (ViewState["bags"] != null) dt = ViewState["bags"] as DataTable;
                                if (ViewState["cns"] != null) dtC = ViewState["cns"] as DataTable;
                                if (dt != null || dtC != null)
                                {
                                    string result = "";
                                    string result_ = "";
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Loading ID Already Inserted')", true);
                    txt_vid.Text = "";
                    if (ViewState["bags"] != null) dt = ViewState["bags"] as DataTable;
                    if (ViewState["cns"] != null) dtC = ViewState["cns"] as DataTable;
                    if (dt != null || dtC != null)
            ...
            else
            {
                ...
                if (dt != null || dtC != null)
                {
                    ...
                    if (flag)
                    {
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "response", "LoadTable('" + result + "','" + result_ + "','" + false + "');", true);
                    }
                }
            }
            //txt_vid.Text = "";
        ...
        protected void gvLoading_OnRowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Del")
            {
                ...
                if (ViewState["dtUnload"] != null)
                {
                    ...
                    for (int i = dtUnload.Rows.Count - 1; i >= 0; i--)
                    {
                        DataRow dr = dtUnload.Rows[i];
                        if (dr["loadingID"].ToString() == e.CommandArgument.ToString())
                            dr.Delete();
                    }
                    dtUnload.AcceptChanges();
                    ...
                    for (int i = dtC.Rows.Count - 1; i >= 0; i--)
                    {
        ...
        void Load_Loding(DataTable dtLoading, DataTable dtBag, DataTable dtCN)
        {
            bool unloaded = false;
            if (dtLoading.Rows.Count > 0)
            {
                gvLoading.DataSource = dtLoading;
                gvLoading.DataBind();

                if (dtBag != null || dtCN != null)
                {
                    string result = "";
                    string result_ = "";
                    bool flag = false;
        ...
        public DataTable GetLoadingBags(Cl_Variables clvar)
        {
            ...
            "       LB.REMARKS REMARKS,\n" +
            "       'EXISTS' BAGSTATUS,\n" +
            "       LB.ULOADINGSTATEID DESCSTATUS, L.ID as LoadingID\n" +
            "  FROM MNP_LOADING L\n" +
        ...
        public DataTable GetConsignmentForUnload(Cl_Variables clvar)
        {
           ...
           "       b2.name DestBranchId,\n" +
           "       lc.Remarks Remarks,\n" +
           "       'EXISTS' CNSTATUS, L.ID as LoadingID\n" +
           "  from mnp_Loading l\n" +
           " inner join MnP_LoadingConsignment lc\n" +
           "    on lc.LoadingId = l.id\n" +
           //" inner join consignment c\n" +
        ...
        public static List<string> PerformUnloading_BAG(Cl_Variables clvar, Variable clvar_, BagModel bags, string sealNum, string hd_master)
        {
            ...
            try
            {
                ...
                string qry = "SELECT * FROM mnp_unloadingref where loadingId = " + bags.loadingID.ToString();
                DataTable dt = new DataTable();
                SqlConnection con = new SqlConnection(clvar.Strcon());
                con.Open();
                ...
                if (dt != null)
                {
                    if (dt.Rows.Count > 0)
                    {
                        if (dt.Rows[0][1].ToString() != unloadingID.ToString())
                        {
                            throw new Exception("Already Unloaded against " + dt.Rows[0][1].ToString().ToString());
                        }
                    }
                    else
                    {
                        refQuery = @"insert into mnp_unloadingref (unloadingID, loadingID, CreatedBy, Createdon ) values (
                    " + unloadingID + ", " + bags.loadingID.ToString() + ",'" + HttpContext.Current.Session["U_ID"].ToString() + "', GETDATE())";
        ...
        public static List<string> PerformUnloading_CN(Cl_Variables clvar, Variable clvar_, ConsignmentModel CN, string sealNum, string hd_master)
        {
            ...
            try
            {
                ...
                string qry = "SELECT * FROM mnp_unloadingref where loadingId = " + CN.LoadingID.ToString();
                DataTable dt = new DataTable();
                SqlConnection con = new SqlConnection(clvar.Strcon());
                con.Open();
                SqlDataAdapter sda = new SqlDataAdapter(qry, con);
                ...
                if (dt != null)
                {
                    ...
                    {
                        refQuery = @"insert into mnp_unloadingref (unloadingID, loadingID, CreatedBy, Createdon )  values (
                    " + unloadingID + ", " + CN.LoadingID.ToString() + @",'" + HttpContext.Current.Session["U_ID"].ToString() + "', GETDATE())";
                        queries.Add(refQuery);
                    }
                }
                else
