...
{
    public partial class StockIssuanceBranchEC : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static List<StockRequestModel> GetRequests(string EC)
        {
            //return new StockIssuanceBranchECRepo().GetRequestsZone(EC,ZoneCode,BranchCode);
            try
            {
                con.Open();
                ...
                if (EC!="")
                {
                    var rs = con.Query<StockRequestModel>($@"select sr.ID as [Id], sr.ZoneCode,b.name branchName,b.branchCode BranchCo,ec.expressCenterCode+' - '+ec.name  ECName,ec.expressCenterCode ECRiderCode, sr.Year, sr.TotalReqQty, FORMAT(sr.TotalReqQty,N'N0') as TotalReqQty_, 
                ...
                and sr.ZoneCode='{HttpContext.Current.Session["ZONECODE"].ToString()}' and sr.BranchCode='{HttpContext.Current.Session["BRANCHCODE"].ToString()}' and sr.ECRiderCode='{EC}' and sr.UserTypeID=1006" );
                    con.Close();
                    return rs.ToList();
                }
                else
                ...
                left join ExpressCenters ec on Ec.expressCenterCode=sr.ECRiderCode
                where sr.isActive = 1  AND si.ID is null AND sr.TotalReqQty > 0 
                and sr.ZoneCode=@ZoneCode and sr.BranchCode=@branch  and sr.UserTypeID=1006", new
                    {
        ...
        public static List<StockModel> GetZoneStock(string EC)
        {
            ...
            {
                con.Open();
                var rs = con.Query<StockModel>(@"Select productID,pr.ProductName,t.TypeName, 
            sum(qty) as Qty, FORMAT(sum(qty),N'N0') as Qty_  from (
        ...
        group by ProductID,ProductName,TypeName,ec.name,xb.BranchID,xb.ECRiderCode
        ...
        {
            //return new StockIssuanceBranchECRepo().GetRequestsDetails(id);
            try
            {
                con.Open();
        ...
        public static string SaveIssuanceDetails(StockIssuanceModel model)
        {
            ...
            object InsertInIssuanceDetail;
            object InsertStockStatusInTransit;
            object InsertStockStatusOut;
            con.Open();

            using (var tran = con.BeginTransaction())
            {
                // return new StockIssuanceBranchECRepo().SaveIssuanceDetails(model,ZoneCode, BranchCode);
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into MnP_CNIssue_StockIssuance (StockReqID,[Year],ZoneCode,[BranchCode],[ECRiderCode],TotalIssueQty,isActive,CreatedBy,CreatedOn,UserType) values(@StockReqID,@Year,@ZoneCode,@BranchCode,@ECRiderCode,@TotalIssueQty,1,@CreatedBy,GETDATE(),1006);;
                                                        SELECT SCOPE_IDENTITY();", new { @StockReqID = model.StockRequestId, @Year = model.Year, @ZoneCode = HttpContext.Current.Session["ZONECODE"].ToString(), @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(), @ECRiderCode = model.ZoneCode, @TotalIssueQty = model.StockIssuanceDetails.Sum(x => x.IssueQty), @CreatedBy = model.CreatedBy }, transaction: tran);

                    foreach (var single in model.StockIssuanceDetails)
                    {
                        InsertInIssuanceDetail = con.Execute(@"insert into MnP_CNIssue_StockIssuanceDetail (StockIssuanceID,StockReqDetailID,IssueQty,BarcodeFrm,BarcodeTo,CreatedBy,CreatedOn,isActive) values(@StockIssuanceID,@StockReqDetailID,@IssueQty,@BarcodeFrm,@BarcodeTo,@CreatedBy,GETDATE(),1);"
                       , new
                       {
                           StockIssuanceID = id,
                        ...
                        InsertStockStatusInTransit = con.Execute(@"insert into[Mnp_CNIssue_Stock] (StatusID,ProductID,ZoneCode,[BranchID],[ECRiderCode],BarcodeFrm,BarcodeTo,Qty,EntryDate,UserTypeID) values(5,@ProductID,@ZoneCode,@BranchID,@ECRiderCode,@BarcodeFrm,@BarcodeTo,@IssueQty,GETDATE(),1006);"
                       , new
                       {
                        ...
                        InsertStockStatusOut = con.Execute(@"Insert into[Mnp_CNIssue_Stock] (StatusID,ProductID,ZoneCode,[BranchID],[ECRiderCode],BarcodeFrm,BarcodeTo,Qty,EntryDate,UserTypeID) values(2,@ProductID,@ZoneCode,@BranchID,@EC,@BarcodeFrm,@BarcodeTo,@IssueQty,GETDATE(),1004);"
                        , new
                        {
                            ProductID = single.ProductId,
                            ZoneCode = HttpContext.Current.Session["ZONECODE"].ToString(),
