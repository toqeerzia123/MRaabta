...
namespace MRaabta.Files
{
    public partial class RecoveryReport_Main : System.Web.UI.Page
    {
        ...
        string DateTimeStarting = "2021-12-01";
...
"CASE WHEN ISNULL(c.DiscountID, '0') = '0' AND SUM(ISNULL(cm.calculatedValue, '0')) = 0 THEN c.totalAmount + c.gst \n" +

"WHEN ISNULL(c.DiscountID, '0') != '0' AND SUM(ISNULL(cm.calculatedValue, '0')) = 0 THEN c.chargedAmount \n" +

"WHEN ISNULL(c.DiscountID, '0') = '0' AND SUM(ISNULL(cm.calculatedValue, '0')) != 0 THEN c.totalAmount + c.gst + SUM(cm.calculatedValue) + SUM(cm.calculatedGST) \n" +

"WHEN ISNULL(c.DiscountID, '0') != '0' AND SUM(ISNULL(cm.calculatedValue, '0')) != 0 THEN c.chargedAmount + SUM(cm.calculatedValue) + SUM(cm.calculatedGST) END GrossAmount, stn.products, \n" +

"C.PaymentMode--CASE WHEN C.PaymentMode = '1' THEN 'CASH' WHEN C.PaymentMode = '6' THEN 'QR CODE' WHEN C.PaymentMode = '7' THEN 'CREDIT CARD' END PaymentMode \n" +

"FROM Consignment c \n" +

"INNER JOIN ServiceTypes_New stn ON stn.serviceTypeName = c.serviceTypeName \n" +

"inner join(select distinct M.DSSPNumber, D.ConsignmentNumber from MNP_Master_Retail_DSSP M inner join MNP_Detail_Retail_DSSP D On D.DSSPNumber = M.DSSPNumber where ExpressCenterCode = '" + ExpressCenterCode + @"' and M.CreatedOn >= '" + DateTimeStarting + @"') M on C.consignmentNumber = M.ConsignmentNumber " +

 " \n LEFT JOIN ConsignmentModifier cm ON cm.consignmentNumber = c.consignmentNumber \n" +

"WHERE c.originExpressCenter = '" + ExpressCenterCode + @"' AND c.status != 9  " +

"\n GROUP BY M.DSSPNumber, c.consignmentNumber, c.DiscountID, c.chargedAmount, c.totalAmount, c.gst, stn.products, C.PaymentMode) d \n" +
