...
namespace MRaabta.Files
{
    public partial class CIH_GLReport : System.Web.UI.Page
    {
        ...
        protected void btn_addhbl_Click(object sender, EventArgs e)
        {
            lbl_error.Text = "";
            DataSet ds = new DataSet();
            try
            {
                ...
                //string year = HttpUtility.UrlEncode(Encrypt(dd_year.SelectedItem.ToString().Trim()));
                //Redirect to the print view page
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "OpenWindow", "window.open('CIH_HBL_GLprint.aspx?from=" + frm_date + "&to=" + to_date + "&branch=" + branch_val + "&bname=" + branch_name + "&company=" + company + "');", true);
