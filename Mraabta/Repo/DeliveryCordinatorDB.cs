...
{
    public class DeliveryCordinatorDB
    {
        SqlConnection orcl;
        public DeliveryCordinatorDB()
        {
            ...
            try
            {
                ...
                var data = new List<Status>
                {
                    new Status
                    {
                        ...
                        delivered = rs.Where(x => x.StatusId == 1).Select(x => x.CN).Distinct().Count(),
                        deliveredRts = rs.Where(x => x.StatusId == 3).Select(x => x.CN).Distinct().Count(),
                        undelivered = rs.Where(x => x.StatusId == 2).Select(x => x.CN).Distinct().Count(),
                        Pending = rs.Where(x => x.StatusId == 0).Select(x => x.CN).Distinct().Count(),
