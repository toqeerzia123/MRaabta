<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/BtsMasterPage.Master" CodeBehind="StockBranchIssuance.aspx.cs" Inherits="MRaabta.Files.StockBranchIssuance" %>
...
<asp:Content ID="form1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
            ...
            <table cellpadding="0" cellspacing="0" width='100%' class='mGrid_Table '>
                <tr>
                    <td colspan="12" align="center" class="head_column">
                        <h3 style="text-align: center">Stocks Issuance To Branch
                        </h3>
                    </td>
                        ...
                        <tr>
                            <div class="row ml-2 mt-2">
                                <div class="form-group row">
                                    <label for="inputEmail3" class="col-sm-2 col-form-label mt-1" style="font-size: 12px;">Branches</label>
                                    <div class="col-sm-5 mt-1">
                                        <select class="ddlbranch  form-control" id="inputState" style="font-size: 12px;">
                                        </select>
                                    </div>
                                </div>
                        ...
                        <td>
                            <nav>
                                <div class="nav nav-tabs nav-fill " id="nav-tab" role="tablist">
                                    <a class="nav-item nav-link active" id="nav-home-tab" data-toggle="tab" href="#nav-home" role="tab" aria-controls="nav-home" aria-selected="true">Zone Stock</a>
                                </div>
                        </nav>
                        </td>
                    </tr>
    ...
    <script type="text/javascript">
        var getBranches = () => {
            ...
            $.ajax({
                type: "POST",
                url: 'StockBranchIssuance.aspx/GetBranches',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    var kk = '';
                    kk += `<option value="0">All Branches</option>`;
                    var rr = rs.d;
                    for (let y of rr) {
        ...
        var getRequests = (id) => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockBranchIssuance.aspx/GetRequests',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                ...
                success: (rs) => {
                    ...
                    for (let x of data) {
                        ...
                        rows += `<tr style="text-align:center;">
                                    ...
                                    <td>
                                        <button type="button"  CssClass="button" class="button btn btn-sm btn-outline-dark btnView" data-bname="${x.BranchName}" data-name="${x.BranchCode}" data-zone="${x.ZoneCode}" value="${x.Id}" data-isupdated="${x.IsUpdated}" >View</button>
                                    </td>
                                </tr>`;
                    }
                    $('#reqGrid').html(rows);
        ...
        var getMyStock = () => {
            $.ajax({
                type: 'post',
                url: 'StockBranchIssuance.aspx/GetMyStock',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    var data = rs.d;
        ...
        var getZoneStock = (zone, branch) => {
            
            $.ajax({
                type: 'post',
                url: 'StockBranchIssuance.aspx/GetBranchStock',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        var getRequestDetails = (id, isupdated, zone, name) => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockbranchIssuance.aspx/GetRequestsDetails',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                data: JSON.stringify({ id: id }),
                success: (rs) => {
                    ...
                    if (isupdated == false) {
                        $('#btnDiv').html(`<button id="btnSubmit" type="button" class="button btn btn-outline-dark" style="float:right;" data-id="${id}" data-zone="${zone}" data-branch="${name}">Submit</button>`);
                    } else {
                        $('#btnDiv').html('');
                    }
        ...
        var save = (data) => {
            $.ajax({
                type: 'post',
                url: 'StockBranchIssuance.aspx/SaveIssuanceDetails',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        $(function () {
            ...
            $('body').on('change', '.ddlbranch', function () {
            ...
            $('body').on('click', '.btnView', function () {
                ...
                getRequestDetails(id, isupdated, zone, branch);
                getZoneStock(zone, branchid);
