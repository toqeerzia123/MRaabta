...
{
    public partial class ReceiptVoucher_RR : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (dd_paySource.SelectedValue != "1" && dd_paySource.SelectedValue != "0")
            {
        ...
        {
            DataTable dt = GetPaymentVoucherTypes();
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = rec.GetPaymentSources();
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
        ...
        {
            string query = "select * from PaymentTypes where id != '5'";
        ...
        {
            //DataTable dt = rec.GetClientGroups();
            //if (dt != null)
            //{
            //    if (dt.Rows.Count > 0)
            //    {
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rbtn_paymentMode.SelectedValue == "2")
                {
                    ...
                    DataTable allowedReceiptNo = SequenceCheck(clvar);
                    if (allowedReceiptNo != null)
                    {
                        if (allowedReceiptNo.Rows.Count <= 0)
                        {
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Invalid Receipt Number')", true);
                            Errorid.Text = "Invalid Receipt Number";
                            return;
                        }
                    }
                    else
                    ...
                    {
                        clvar.ClientGroupID = txt_clientGroup.Text.Trim(); // dd_group.SelectedValue;
                ...
                else
                {
                    ...
                    else if (rbtn_byRider.Checked && dd_rider.SelectedValue == "0")
                    {
                        Errorid.Text = "Select Rider";
                        return;
                    }
                    ...
                    {
                        clvar.riderCode = dd_rider.SelectedValue;
        ...
        protected void txt_voucherNo_TextChanged(object sender, EventArgs e)
        {
            ...
            if (ds.Tables[0].Rows.Count != 0)
            {
                ...
                {
                    dd_rider.SelectedValue = ds.Tables[0].Rows[0]["RiderCode"].ToString();
                }
        ...
        public DataTable SequenceCheck(Cl_Variables clvar)
        {
            ...
            "  FROM Mnp_BranchRRSequence mbr\n" +
            " WHERE '" + clvar.ReceiptNo + "' BETWEEN mbr.StartSequence AND mbr.EndSequence\n" +
            "   AND LEN('" + clvar.ReceiptNo + "') = LEN(mbr.StartSequence)\n" +
            "   AND mbr.Branch = '" + HttpContext.Current.Session["BranchCode"].ToString() + "' and mbr.status = '1'";
