...
{
    public partial class StockRequestRider_IMS : System.Web.UI.Page
    { 
        ...
        { 
            try
            {
                ...
                var rs = con.Query<CNIssuanceDropDownModel>(@"SELECT  r.riderCode as [Value],
                        ...
                        where r.branchId =@branchCode and r.status = '1' AND rt.[status]=1", new { @branchCode = HttpContext.Current.Session["BRANCHCODE"].ToString() });
                con.Close();
                return rs.ToList();
            }
            catch (SqlException ex)
