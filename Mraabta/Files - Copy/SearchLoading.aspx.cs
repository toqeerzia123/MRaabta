...
{
    public partial class SearchLoading : System.Web.UI.Page
    {
        ...
        public DataSet Get_SearchLoading(Variable clvar)
        {
            ...
            try
            {
                string sql = " select l.id,lu.AttributeDesc TransportType, \n" +
                             ...
                             "        (SUM(CAST(ISNULL(lb.BagWeight, 0) as float)) + SUM(CAST(ISNULL(lc.cnWeight, 0) as float))) TotalWeight \n" +
                             "   from mnp_Loading l \n" +
                             ...
                             "   left outer join MnP_LoadingBag lb \n" +
                             "     on lb.loadingId = l.id \n" +
                             "   left outer join MnP_LoadingConsignment lc\n" +
                             "     on lc.loadingId = l.id\n" +
                             " WHERE l.branchCode = '" + HttpContext.Current.Session["BranchCode"].ToString() + "' " + clvar.Check_Condition + "\n" +
                             " group by l.id,lu.AttributeDesc , \n" +
                             "       CONVERT(VARCHAR(10), l.date, 105) ,\n" +
               ...
               "       l.sealno,\n" +
               "       l.departureflightdate,\n" +
               "       ((SELECT SUM(CAST(ISNULL(lb.BagWeight, 0) as float))\n" +
               "           from mnp_LoadingBag lb\n" +
               "          where lb.loadingId = l.id) +\n" +
               "       (SELECT SUM(CAST(ISNULL(lb.CNWeight, 0) as float))\n" +
               "           from MnP_LoadingConsignment lb\n" +
               "          where lb.loadingId = l.id)) TotalWeight, \n" +
               "       u.Name as CreatedBy" +
               "  from mnp_Loading l\n" +
               "  left outer join rvdbo.Lookup lu\n" +
               "    on lu.id = l.transportationType\n" +
               ...
               " WHERE l.branchCode = '" + HttpContext.Current.Session["BranchCode"].ToString() + "' " + clvar.Check_Condition + "\n";
                ...
                //if (clvar.LoadingId != "")
                //{
                //    sql += " where l.id = '" + clvar.LoadingId + "' and l.branchCode = '" + HttpContext.Current.Session["BranchCode"].ToString() + "' " + clvar.Check_Condition + "";


                //}
                //else if (clvar._StartDate != "")
                //{
                //    sql += " where CONVERT(VARCHAR(10), l.date, 105)='" + clvar._StartDate + "' and l.branchCode = '" + HttpContext.Current.Session["BranchCode"].ToString() + "' " + clvar.Check_Condition + " ";
                //}
                //else
                //{
                //}
