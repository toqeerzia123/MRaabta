...
{
    public partial class StockBranch : System.Web.UI.Page
    {
        ...
        {
            //return new StockRequestBranchRepo().GetZoneStock(ZoneCode);

            try
            {
                con.Open();
                var rs = con.Query<StockModel>(@"
            select productID,pr.ProductName,t.TypeName,z.name as Zone, 
            sum(qty) as Qty, 
        ...
        {
            //return new StockRequestBranchRepo().GetBranchStock(BranchCode, ZoneCode);
            try
            {
                con.Open();
        ...
        public static List<CNIssuanceDropDownModel> GetProductsSingle(String Id)
        {
            ...
            //var rs = new StockRequestBranchRepo().GetProductsSingle(Id);
            try
            {
                con.Open();
        ...
        {
            //var rs = new StockRequestBranchRepo().GetTypeProducts();
        ...
        {
            //return new StockRequestBranchRepo().IssuedRequest(ZoneCode);
            try
            {
                con.Open();
        ...
        {
            //return new StockRequestBranchRepo().GetPendingRequestDetails(Id);
            try
            {
                con.Open();
                var rs = con.Query<StockModel>(@"
        ...
        {
            //return new StockRequestBranchRepo().PendingRequest(ZoneCode);
            try
            {
                con.Open();
        ...
        public static Response_StockRequestDetailModel GetFilteredStockZone(String StartDate, String EndDate)
        {
            ...
            try
            {
                ...
                con.Open();
                //        var rs = con.Query<StockRequestDetailModel>(@"Select sred.ID DetailId,sred.StockReqID ReqId,sred.Qty,ty.TypeName TypeName,sr.CreatedOn,pr.ProductName  ProductName from MnP_CNIssue_StockReqDetails sred 
                //            LEFT JOIN MnP_CNIssue_StockRequest sr on sred.StockReqID=sr.ID
                //            LEFT JOIN MnP_CNIssue_Product pr on pr.ID=sred.ProductID
                //            LEFT JOIN MnP_CNIssue_Type ty on pr.TypeId=ty.ID
                //LEFT JOIN MnP_CNIssue_StockIssuanceDetail sid on sid.StockReqDetailID = sred.ID
                //            where sr.BranchCode=@BranchCode and sid.ID is null and sr.CreatedOn between @startt +' 00:00:00.000' and @endd+' 23:59:59.999 ';
                //            ", new { @BranchCode = BranchCode, @startt = StartDate, @endd = EndDate });

                var rs = con.Query<StockRequestDetailModel>(@"Select sr.ID ReqId,sr.TotalReqQty Qty,  FORMAT(sr.TotalReqQty,N'N0') as Qty_  , CONVERT(VARCHAR(11),                   
                ...
                where sr.ZoneCode=@Zonee and sr.BranchCode=@Branch  and UserTypeID=1004 and sid.ID is null and sr.CreatedOn between @startt +' 00:00:00.000' and @endd+' 23:59:59.999 ';
        ...
        {
            //return new StockRequestBranchRepo().GetFilteredStockZonePer(Id);        
            try
            {
                con.Open();
                var rs = con.Query<StockRequestDetailModel>(@"select sred.ID DetailId,sred.StockReqID ReqId,sred.Qty,ty.TypeName,ty.ID TypeId,pr.ID ProductId,pr.ProductName  Product
                from MnP_CNIssue_StockRequest sr 
        ...
        public static string SaveStockDetails(StockRequestModel model)
        {
            ...
            //var rs = new StockRequestBranchRepo().SaveStockDetails(model, ZoneCode, BranchCode);
            con.Open();
            object reqDetails;
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into [MnP_CNIssue_StockRequest] ([Year],ZoneCode, BranchCode, UserTypeID, TotalReqQty,isActive,CreatedBy,CreatedOn) values(@Year,@zonecode,@BranchCode,1004,@TotalReqQty,1,@CreatedBy,GETDATE());
                                                        SELECT SCOPE_IDENTITY();", new { @Year = DateTime.Now.Year, @ZoneCode = HttpContext.Current.Session["ZONECODE"].ToString(), @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(), @UserTypeID = model.UserTypeId, @TotalReqQty = model.StockRequestDetails.Sum(x => x.Qty), @CreatedBy = HttpContext.Current.Session["U_ID"].ToString() }, transaction: tran);

                    foreach (var singleDetail in model.StockRequestDetails)
                    {
                        reqDetails = con.Execute(@"insert into [MnP_CNIssue_StockReqDetails] (StockReqID,ProductID, Qty,isActive,CreatedBy,CreatedOn) values(@StockReqID,@ProductID, @Qty,1,@CreatedBy,GETDATE());",
                        new
                        {
                            StockReqID = id,
