...
{
    public partial class Manage_ConsignmentApproval_QA : System.Web.UI.Page
    {
        ...
        protected DataTable GetAccessProfiles()
        {
            string sql = "SELECT pd.Profile_Id \n"
               ...
               + "WHERE  pd.ChildMenu_Id in ('67', '60', '260') and pd.profile_id = '" + HttpContext.Current.Session["Profile"].ToString() + "'";
        ...
        public DataSet ServiceTypeName()
        {
            ...
            try
            {
                ...
                + "WHERE  \n"
                + "       st.[status] = '1' AND serviceTypeCategory <> 'Import'\n"
        ...
        public DataSet ConsignmentType()
        {
            ...
            try
            {
                ...
                + "       WHERE ct.[status]='1' and id not in ('18','19') \n"
        ...
        public DataSet PriceModifiers_()
        {
            DataSet Ds_1 = new DataSet();
        ...
        protected void txt_cnNumber_TextChanged(object sender, EventArgs e)
        {
            ...
            pm_.Columns.Add("ModifierPieces");
            pm_.AcceptChanges();
            ViewState["pm"] = pm_;
            ...
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
                    hd_misroute.Value = dt.Rows[0]["misRouted"].ToString();
                    hd_BookingDate.Value = dt.Rows[0]["BookingDate"].ToString();
                    hd_ShipperAddress.Value = dt.Rows[0]["shipperAddress"].ToString();
                    hd_PakageContents.Value = dt.Rows[0]["PakageContents"].ToString();
                    ...
                    hd_inserttype.Value = dt.Rows[0]["InsertType"].ToString();
                    ...
                    if (dt.Rows[0]["orgin"].ToString() != HttpContext.Current.Session["BranchCode"].ToString())
                    {
                        ...
                        //ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Consignment Not allowed in your Zone.')", true);
                        //return;
                    }
                    else
                    {
                    ...
                    if (dt.Rows[0]["PaymentMode"].ToString() != null && dt.Rows[0]["PaymentMode"].ToString() != "")
                    {
                        dd_PaymentMode.SelectedValue = dt.Rows[0]["PaymentMode"].ToString();
                    }
                    else
                    {
                        dd_PaymentMode.SelectedValue = "0";
                    }
                    ...
                    foreach (DataRow row in dt.Select("priceModifierId is not null", ""))
                    {
                        ...
                        dr["calculatedValue"] = row["calculatedValue"];
                        ...
                        dr["AlternateValue"] = row["AlternateValue"];
                        dr["ModifierPieces"] = row["ModifierPieces"];
                        pm.Rows.Add(dr);
                        pm.AcceptChanges();
                    }
        ...
        {
            //if (dd_origin.SelectedValue != HttpContext.Current.Session["BranchCode"].ToString())
            //{
            //    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Consignment Not allowed in your Zone.')", true);
            //    return;
            //}
                ...
                if (dt != null)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        if (clvar.ServiceTypeName != "Road n Rail" && clvar.ServiceTypeName != "Express Cargo")
                        {
                            ...
                            if (dd_consignmentType.SelectedValue == "13" && clvar.ServiceTypeName != "Road n Rail" && clvar.ServiceTypeName != "Express Cargo")
                            {
                                clvar.CustomerClientID = hd_CreditClientID.Value;
        ...
        protected void btn_add_Click(object sender, EventArgs e)
        {
            ...
            else if (dd_calculationBase.SelectedValue == "3" && tempDeclaredValue > 300000)
            {
                AlertMessage("Declared Value must be lesser than 300,000");
                return;
            }
            ...
            if (dd_priceModifier.SelectedValue.Split('-')[0] == "111" || dd_priceModifier.SelectedValue.Split('-')[0] == "112" ||
                    dd_priceModifier.SelectedValue.Split('-')[0] == "113" || dd_priceModifier.SelectedValue.Split('-')[0] == "114" ||
                    dd_priceModifier.SelectedValue.Split('-')[0] == "115" || dd_priceModifier.SelectedValue.Split('-')[0] == "116" ||
                    dd_priceModifier.SelectedValue.Split('-')[0] == "117")
            {

                if (float.Parse(txt_PiecesModifiere.Text) > 0)
                {
                    // txt_modifier_pieces.te
                    double a = double.Parse(dd_priceModifier.SelectedValue.Split('-')[2]) * double.Parse(txt_PiecesModifiere.Text);
                    dr["calculatedValue"] = a;
                    dr["ModifierPieces"] = float.Parse(txt_PiecesModifiere.Text);
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Cannot Add Zero Pieces')", true);
                    return;
                }
            }
            else
            ...
            dr["ISTAXABLE"] = pm.Select("id = '" + dd_priceModifier.SelectedValue.Split('-')[0] + "'")[0]["ISgst"].ToString();
                    ...
                    dd_priceModifier.SelectedValue.Split('-')[0] == "117")
            {
                ...
                for (int count = 0; count < gv_CNModifiers.Rows.Count; count++)
                {


                    //to get the template field value
                    TextBox tx1 = (TextBox)gv_CNModifiers.Rows[count].FindControl("txt_gValue");
                    tx1.Enabled = false;
                }
            }
        }
        ...
        protected void dd_priceModifier_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (dd_priceModifier.SelectedValue != "0")
            {
                ...
                txt_PiecesModifiere.Enabled = false;
                txt_PiecesModifiere.Text = "";
                if (dd_calculationBase.SelectedValue == "3")
                {
                    txt_declaredValue.Enabled = true;
                }
                else if (dd_priceModifier.SelectedValue.Split('-')[0] == "111" || dd_priceModifier.SelectedValue.Split('-')[0] == "112" ||
                    dd_priceModifier.SelectedValue.Split('-')[0] == "113" || dd_priceModifier.SelectedValue.Split('-')[0] == "114" ||
                    dd_priceModifier.SelectedValue.Split('-')[0] == "115" || dd_priceModifier.SelectedValue.Split('-')[0] == "116" ||
                    dd_priceModifier.SelectedValue.Split('-')[0] == "117")
                {
                    txt_PiecesModifiere.Text = "1";
                    txt_declaredValue.Text = "";
                    txt_declaredValue.Enabled = false;
                    txt_PiecesModifiere.Enabled = true;
                }
                else
                {
                    txt_declaredValue.Enabled = false;
        ...
        {
            #region OLD
            string sqlString__ = "select MAX(DateTime) DateAllowed from Mnp_Account_DayEnd where Branch = '" + clvar.Branch + "' and doc_type = 'A'";
            string sql__ = "SELECT MAX(d.DateTime)        DateAllowed \n"
        ...
        public DataTable GetConsignmentForApproval(Cl_Variables clvar)
        {
            ...
            "       cm.isTaxable,\n" +
            "       cm.SortOrder,\n" +
            "       p.description, c.destinationExpressCenterCode, c.couponNumber, c.remarks, cm.AlternateValue, c.paymentmode, c.paymentTransactiontid\n" +
            "\n" +
            "  from consignment c\n" +
            ...
            "         c.consignmentNumber,c.InsertType,c.misRouted,\n" +
            "         c.customerType,\n" +
            "         c.creditClientId,\n" +
            "         c.orgin,\n" +
            "         c.serviceTypeName,\n" +
            ...
            "         --    i.startDate                       ReportingDate,\n" +
            "         --    i.deliveryStatus,\n" +
            "         cm.priceModifierId,cm.Pieces ModifierPieces,\n" +
            "         p.name priceModifierName,\n" +
            "         cm.calculatedValue,\n" +
            "         cm.modifiedCalculationValue modifiedCalculatedValue,\n" +
            "         cm.calculationBase,\n" +
            ...
            //"            '1'\n" +
            //"         end PortalCN, \n" +
            "           cns.CreditClientID is not null AND cc.IsCOD = '1' then \n" + 
            "            '1'\n" +
            "           else\n" +
            "            '0'\n" +
            "         end PortalCN, c.width, c.breadth, c.height, c.denseWeight, c.remarks, cm.AlternateValue, c.paymentmode, c.paymentTransactiontid\n" +
            "          FROM consignment c\n" +
            "          LEFT OUTER JOIN InvoiceConsignment ic\n" +
            "            ON c.consignmentNumber = ic.consignmentNumber\n" +
            "        --LEFT OUTER JOIN Invoice i\n" +
            ...
            "               and c.consignmentTypeId <> '10'\n" +
            "               and */\n" +
            "         c.consignmentNumber = '" + clvar.consignmentNo.Trim() + "'\n" +
            "        --     AND ISNULL(i.IsInvoiceCanceled, 0) = '0'\n" +
            "        ) A\n" +
            "  LEFT OUTER JOIN Invoice i\n" +
            "    ON A.invoiceNumber = i.invoiceNumber\n" +
     ...
     "'" + clvar.consignmentNo + "', GETDATE(), '1', '" + HttpContext.Current.Session["U_ID"].ToString() + "')";
        ...
        protected void btn_SaveConsignment1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                clvar.BaseCurrency = hd_misroute.Value;
                ...
                clvar.ConsignerAddress = hd_ShipperAddress.Value;
                clvar.PakageContents = hd_PakageContents.Value;
                ...
                clvar.instruction = hd_inserttype.Value;
                ...
                //    clvar.productDescription = txt_descriptionCOD.Text;
                //    if (dd_productType.Items.Count > 1)
                //    {
                //        clvar.productTypeId = int.Parse(dd_productType.SelectedValue);
                //    }
                //    else
                ...
                if (dd_serviceType.SelectedValue != "Road n Rail" && clvar.ServiceTypeName != "Express Cargo")
                {
                ...
                new DataColumn("ModifierPieces", typeof(string))
            });
                ...
                foreach (GridViewRow row in gv_CNModifiers.Rows)
                {
                    ...
                    int calBase = int.Parse(row.Cells[5].Text);
                    //string isGST = row.Cells[4].Text;
                    string isGST = row.Cells[6].Text;
                    ...
                    dr["modifiedCalculationValue"] = float.Parse((row.FindControl("txt_modifiedCalculationValue") as TextBox).Text);
                    //dr["modifiedCalculationValue"] = float.Parse((row.FindControl("txt_gValue") as TextBox).Text);
                    //dr["ModifierPieces"] = float.Parse((row.FindControl("txt_modifier_pieces") as TextBox).Text);
                    string aaa = (row.FindControl("txt_modifier_pieces") as TextBox).Text;
                    if (aaa == "")
                    {
                        dr["ModifierPieces"] = 0;
                    }
        ...
        public string Add_Consignment(Cl_Variables obj, float denseWeight, string remarks, string isPriceComputed, string isApproved)
        {
            ...
            try
            {
                ...
                if(clvar.BaseCurrency.ToUpper() == "TRUE")
                {
                    sqlcmd.Parameters.AddWithValue("@misRouted", 1);
                }
                if (clvar.instruction == "2")
                {
        ...
        {
            string sql = @"SELECT
                ID, ZoneCode,SequenceStart,EndSequence
                ...
                UNION ALL
                SELECT
                ID,ZoneCode,BarcodeFrm SequenceStart, BarcodeTo EndSequence       
                FROM Mnp_CNIssue_Stock m 
                WHERE  m.ZoneCode='" + clvar.Zone + @"' AND
                '" + clvar.consignmentNo + @"' BETWEEN m.BarcodeFrm AND m.BarcodeTo ";
