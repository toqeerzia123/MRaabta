<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/BtsMasterPage.Master" CodeBehind="StockIssuanceRider_IMS.aspx.cs" Inherits="MRaabta.Files.StockIssuanceRider_IMS" %>
...
<asp:Content ContentPlaceHolderID="ContentPlaceHolder1" ID="Content1" runat="server" class="mt-5">
            ...
            <table cellpadding="0" cellspacing="0" width='100%' class='mGrid_Table '>
                <tr>
                    <td colspan="12" align="center" class="head_column">
                        <h3 style="text-align: center">Stocks Issuance To Rider
                        </h3>
                    </td>
            ...
            <div class="shadow rounded">
                <div class="">
                    ...
                    <div class="row ml-2 mt-2">
                        <div class="col-md-5">
                            <div class="form-group">
                                <label for="inputEmail3" class="col-form-label mt-1" style="font-size: 12px;">Rider</label>
                                <select class="ddlRider  form-control" id="inputState" style="font-size: 12px;">
                                </select>
                            </div>
                        </div>
                ...
                <table id="ReqGrid" class="MainTbl table table-striped">
                    <thead class="">
                        <tr style="font-size: 12px;">
                            <th style="width: 30%; text-align: center;">Rider</th>
    ...
    <script type="text/javascript">
        ...
        var getRiders = () => {
            ...
            $.ajax({
                type: "POST",
                url: 'StockIssuanceRider_IMS.aspx/GetRider',
        ...
        var getRequests = (id, startdate, enddate) => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockIssuanceRider_IMS.aspx/GetRequests',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                //processData: false,
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm == true) {
                        $.ajax({
                            type: "POST",
                            url: 'StockIssuanceRider_IMS.aspx/DeleteStockRequest',
                            contentType: "application/json; charset=utf-8",
        ...
        var getMyStock = () => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockIssuanceRider_IMS.aspx/GetMyStock',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: (rs) => {
                    var data = rs.d;
        ...
        var getZoneStock = (zone) => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockIssuanceRider_IMS.aspx/GetZoneStock',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        var getRequestDetails = (id, isupdated, rider, name) => {

            $.ajax({
                type: 'post',
                url: 'StockIssuanceRider_IMS.aspx/GetRequestsDetails',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                ...
                success: (rs) => {
                    ...
                    for (let x of data) {
                        rows += `<tr>
                                    ...
                                    <td>${(isupdated == false && (x.BarcodeIssueFrom == null || x.BarcodeIssueFrom == "0")) ? `<input type="text" style="height:2em; font-size:12px;" class="form-control txtbarcodefrom" readonly="readonly">` : `<input type="text" style="height:2em; font-size:12px;" class="form-control txtbarcodefrom" readonly="readonly" value="${x.BarcodeIssueFrom}">`}</td>
...
<br/>
        ...
        var save = (data) => {
            ...
            $.ajax({
                type: 'post',
                url: 'StockIssuanceRider_IMS.aspx/SaveIssuanceDetails',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        $(function () {
            ...
            $('body').on('click', '.btnView', function () {
                ...
                rider = $(this).data('zone');
                var name = $(this).data('name');
                getRequestDetails(id, isupdated, rider, name);
                getZoneStock(rider);
            ...
            $('body').on('blur', '.txtissueqty', function () {
                ...
                if (issueqty > qty) {
                    ...
                    $(this).focus();
                    $('#error-msg').html('Quantity is greater than requested quantity');
                    return;
                }
                ...
                if (issueqty <= 0) {
                    $(this).addClass('is-invalid');
                    $(this).focus();
                    $('#error-msg').html('Quantity is invalid');
                    return;
                }
                else {
                    $('#error-msg').html("");
                    ...
                    if (barcodefrom > 0) {
                        ...
                        $(this).parent().parent().find('td:eq(6)').find('input[type=text]').val(0);
                    }
                }
            });
                ...
                for (let x of rows) {
                    ...
                    if (row.find('td:eq(0)').find('input[type=checkbox]').is(":checked") == true) {
                        if (qty > requestedQty) {
                            ...
                            $('#error-msg').val('Quantity is greater than requested quantity');
                            return;
                        } else {
                        ...
                        if (isNaN(barcodefrom) || barcodefrom < 0) {
                            ...
                            $('#error-msg').html('barcode is invalid');
                            return;
                        } else {
                        ...
                        if (qty > qtyIssueFrom) {
                            row.find('td:eq(5)').find('input[type=text]').addClass('is-invalid');
                            row.find('td:eq(5)').find('input[type=text]').focus();
                            $('#error-msg').html('Quantity is greater than the available quantity');
                            return;
                        } else {
                            row.find('td:eq(5)').find('input[type=text]').removeClass('is-invalid');
                        }
