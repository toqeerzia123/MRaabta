...
{
    public class ConsignmentDB_v5
    {

        SqlConnection con;

        public ConsignmentDB_v5()
        {
        ...
        public async Task<ConsignmentStatus> insertDB(ConsginmentModel consignobjs)
        {
            ...
            try
            {
                ...
                if (consignobjs.mobileSyncTime == null || consignobjs.mobileSyncTime==DateTime.MinValue)
                {
                    query = @"insert into App_Delivery_ConsignmentData (ConsignmentNumber,RunSheetNumber,riderCode,name,phone_no,is_nic,
                     ...
                     rs = await con.ExecuteAsync(query, new
                    {
                ...
                else
                {
                    query = @"insert into App_Delivery_ConsignmentData (ConsignmentNumber,RunSheetNumber,riderCode,name,phone_no,is_nic,
                            is_self,is_cod,cod_amount,created_by,created_on,reason,relation,picker_name,
                            latitude,longitude,delivery_id,nic_number,isMobilePerformed,rider_comments,pickerPhone_No,rider_iemi,rider_amount_entered,StatusId,Battery, performed_on,ReasonId,RelationId,mobileSyncTime)
                            values (@ConsignmentNumber,@RunsheetNum,@riderCode,@name,@phone_no,@is_nic,@is_self,@is_cod,@cod_amount,@createdby, CAST(getdate() AS datetime),@reason,@relation,@picker_name,@latitude,@longitude,@deliveryId,@nic_num,@isMobilePerformed,@rider_comments,@pickerPhone_No,@rider_iemi,@rider_amount_entered,@statusId,@battery, @performedOn,@ReasonId,@RelationId,@mobileSyncTime)";
                     ...
                     rs = await con.ExecuteAsync(query, new
                    {
                        ...
                        @ReasonId = consignobjs.reasonId == 0 ? null : consignobjs.reasonId,
                        @RelationId = consignobjs.relationId == 0 ? null : consignobjs.relationId,
                        @mobileSyncTime = consignobjs.mobileSyncTime
                    });
                }
