...
{
    public partial class EC_Consumption_IMS : System.Web.UI.Page
    { 
        ...
        [WebMethod]
        public static List<StockConsumptionModel> GetECConsumption()
        {
            try
            {
                con.Open();
                ...
                group by pr.OracleCode,ProductID, xb.ZoneCode,BranchCode,xb.ECRiderCode,ProductName,TypeName,name
                having xb.ZoneCode = @Zone and xb.BranchCode = @Branch and xb.ECRiderCode = @EC", new
                {
        ...
        public static List<StockModel> GetECStock()
        {
            try
            {
                ...
                var rs = con.Query<StockModel>(@"
                   					select pr.OracleCode, xb.productID,pr.ProductName ProductName,t.TypeName TypeName,
                    xb.BranchID,xb.ECRiderCode,z.name as Zone, -- FORMAT(sum(qty),N'N0') as Qty_,
                    sum(qty) as Qty, FORMAT(sum(qty),N'N0') - ISNULL(sc.ConsumedQty,0) Qty_              
                    from
					( 
...
                     	 (
					     SELECT ds.productId,ISNULL(SUM(ds.qty),0)  AS ConsumedQty from MnP_Stock_Consumption d
						 INNER JOIN MnP_Stock_ConsumptionDetails ds ON d.Id=ds.ConsumeId
						 where d.UserTypeID = 1006 AND d.ZoneCode=@Zone AND d.BranchCode=@Branch
						 AND d.ECRiderCode=@EC  AND ds.ProductID=productid Group by ds.productId
						  )  sc on sc.productId=pr.ID                   
					inner join [MnP_CNIssue_Type] t on t.id = pr.TypeId
					inner join Zones z on xb.ZoneCode = z.zoneCode
                    inner join Branches b on xb.BranchID = b.branchCode
                    group by pr.OracleCode,xb.ProductID, xb.ZoneCode,ProductName,TypeName,z.name,xb.BranchID,xb.ECRiderCode, sc.ConsumedQty 
        ...
        public static string ValidateQuantity()
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                ...
                {
                    con.Open();
                    var rs = con.QueryFirstOrDefault<string>(@"
                        SELECT 
                        ( 
                            select isnull(SUM(qty),0) Qty from Mnp_CNIssue_Stock
				            where UserTypeID = 1006 AND ZoneCode=@Zone AND branchid=@Branch 
				            AND ECRiderCode=@EC AND ProductID='8'               
                        ) - consumed.ConsumedQty RemainingStock FROM 
                        ( 
                        select isnull(SUM(ds.qty),0)  AS ConsumedQty
                        from MnP_Stock_Consumption d
                        INNER JOIN MnP_Stock_ConsumptionDetails ds ON d.Id=ds.ConsumeId
                        where d.UserTypeID = 1006 AND d.ZoneCode=@Zone AND d.BranchCode=@Branch 
                        AND d.ECRiderCode=@EC  AND ds.ProductID=@Product               
                        ) as consumed  ", new
                    {
                        @Zone = HttpContext.Current.Session["ZONECODE"].ToString(),
                        @Branch = HttpContext.Current.Session["BRANCHCODE"].ToString(),
        ...
        public static string SaveStockDetails(StockRequestModel model)
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    ...
                    ([Year],ZoneCode,BranchCode,ECRiderCode,UserTypeID,ConsumedTotalQty,isActive,CreatedBy,CreatedOn) 
                    values(@Year,@ZoneCode,@BranchCode,@ECRiderCode,1006,@ConsumedTotalQty,1,@CreatedBy,GETDATE());
                    SELECT SCOPE_IDENTITY();", new
                    {
                        ...
                        @ECRiderCode = HttpContext.Current.Session["ExpressCenter"].ToString(),
                        @ConsumedTotalQty = model.StockRequestDetails.Sum(x => x.Qty),
                        @CreatedBy = model.CreatedBy
                    }, transaction: tran);

                    foreach (var single in model.StockRequestDetails)
                    {
                        rs = con.Execute(@"insert into [MnP_Stock_ConsumptionDetails] 
                        (ConsumeId,ProductID, Qty,isActive,CreatedBy,CreatedOn) 
                        values (@ConsumeId,@ProductID, @Qty,1,@CreatedBy,GETDATE());"
                        , new
                        {
                            ConsumeId = id,
                ...
                {
                    tran.Rollback();
                    return "Error, Request could not be sent, please contact IT for the support";
                }
                finally
                {
                    con.Close();
