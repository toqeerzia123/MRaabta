...
{
    public partial class StockRequestAccount_IMS : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static List<CNIssuanceDropDownModel> GetAccount(string AccountNo)
        {
            try
            {
                con.Open();
...
WHERE cc.AccountNo= @AccountNo and cc.branchCode = @branchCode and status = '1' ;", 
new { @AccountNo = AccountNo, @branchCode = HttpContext.Current.Session["BRANCHCODE"].ToString() });
                con.Close();
                return rs.ToList();
            }
            catch (SqlException ex)
...
Sr.CreatedOn, 106) CreatedOn ,  sr.ECRiderCode+' - '+cc.name   as TypeName
...
where sr.ZoneCode=@Zonee and sr.BranchCode=@Branch and sr.UserTypeID=1007 and sid.ID is null and sr.CreatedOn between @startt +' 00:00:00.000' and @endd+' 23:59:59.999 ';
        ...
        public static string SaveStockDetails(StockRequestModel model)
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into [MnP_CNIssue_StockRequest] ([Year],ZoneCode,BranchCode,ECRiderCode,TotalReqQty,isActive,CreatedBy,CreatedOn,UserTypeID) values(@Year,@ZoneCode,@BranchCode,@RiderCode,@TotalReqQty,1,@CreatedBy,GETDATE(),1007);
