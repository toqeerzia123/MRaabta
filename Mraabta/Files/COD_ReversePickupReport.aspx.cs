...
{
    public partial class COD_ReversePickupReport : System.Web.UI.Page
    {
        ...
        protected void ExportToExcel(CODPickupSearch fields)
        {
            try
            {
                DataTable Year_check = GetMainData(fields.startDate, fields.endDate, fields.Zone, fields.Account, fields.Status);
                ...
                if (Year_check.Rows.Count > 0)
                {
                    ...
                    Response.AddHeader("content-disposition",
                     "attachment;filename=COD_ReversePickupReport.csv");
                    Response.Charset = "";
        ...
        public static string Search_CSV_Click(string DateStart, string DateEnd, string Zone, string Account, string Status, string OutputType)
        {
            try
            {
                ...
                string redirectUrl = "COD_ReversePickupReport.aspx?xy=" + encryp;
                return redirectUrl;
            }
            catch (Exception er)
            {
                return "COD_ReversePickupReport.aspx";
            }
        }
            ...
            try
            {
                ...
                for (int j = 0; j < ds.Rows.Count; j++)
                {
                    ...
                    list.Add(new CODPickupResponse
                    {
                        ...
                        bookingDate = ds.Rows[j]["bookingDate"].ToString(),
                        orderRefNo = ds.Rows[j]["orderRefNo"].ToString(),
                        consignmentNumber = ds.Rows[j]["consignmentNumber"].ToString(),
                        consignee = ds.Rows[j]["consignee"].ToString(),
                        consigneePhoneNo = ds.Rows[j]["consigneePhone"].ToString(),
                        ADDRESS = ds.Rows[j]["ADDRESS"].ToString(),
                        weight = ds.Rows[j]["weight"].ToString(),
                        pieces = ds.Rows[j]["pieces"].ToString(),
                        STATUS = ds.Rows[j]["STATUS"].ToString(),
                        Arrivaldt = ds.Rows[j]["Arrivaldt"].ToString(),
                        deliverydate = ds.Rows[j]["deliverydate"].ToString(),
                        ShipperAddress=ds.Rows[j]["shipperAddress"].ToString()
                        ,CustomerPhone=ds.Rows[j]["CustomerPhone"].ToString()
                        ,BookingDate=ds.Rows[j]["BookingDate"].ToString()
            ...
            try
            {
                ...
                string sql = @"  
                    SELECT  
                           ...
                           CAST(isnull(c.consigner,'') AS VARCHAR) consignee,c.shipperAddress ADDRESS ,
                           ...
                           CAST(isnull(consignee,'') AS VARCHAR)  consigner,
                           CAST(isnull(consigneePhoneNo,'') AS VARCHAR)  CustomerPhone ,
                            CASt(c.consignerPhoneNo AS VARCHAR)  ConsigneePhone,
                           CAST(isnull(c.ADDRESS,'') AS VARCHAR)   shipperAddress,
                           CAST(isnull(c.weight,'') AS VARCHAR)  WEIGHT,
                           CAST(isnull(c.pieces,'') AS VARCHAR)  pieces,CAST(c.BookingDate AS VARCHAR) BookingDate,
                           CASE 
                    ...
                    WHERE   C.serviceTypeName = 'Reversal COD' 
                             AND CAST(c.createdOn AS DAte) BETWEEN CAST('" + dtStart + @"' AS date) AND CAST('"+ dtEnd + @"' AS date)
                           " + queryCondition2 + @"
                           " + queryCondition1 + @"                  
                           " + queryCondition3 
        ...
        public static List<SearchModel> GetAccounts(string Zone)
        {
            ...
            try
            {
                string sql = "";
                if (Zone.ToUpper()=="ALL") {
                   sql = @"   SELECT DISTINCT cc.accountNo,cc.accountNo+'-'+cc.name name, cc.name as cname   FROM CreditClients cc where cc.STATUS=1 AND cc.CODType=1 order by cc.name  ";
                }
                else
                {
                    sql = @"   SELECT DISTINCT cc.accountNo,cc.accountNo+'-'+cc.name name, cc.name as cname    FROM CreditClients cc where cc.STATUS=1 AND cc.CODType=1 order by cc.name  ";
        ...
        public static List<SearchModel> GetZones()
        {
            ...
            try
            {
                string sql = @" SELECT ZoneCode,name FROM Zones WHERE STATUS=1 AND Region IS NOT null order by name";
