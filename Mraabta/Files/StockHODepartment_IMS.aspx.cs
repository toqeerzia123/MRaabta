...
{
    public partial class StockHODepartment_IMS : System.Web.UI.Page
    {
        ...
        public static string SaveStockDetails(StockRequestModel model)
        {
            ...
            using (var tran = con.BeginTransaction())
            {
                try
                {
                    var id = con.QueryFirstOrDefault<int>(@"insert into [MnP_CNIssue_StockRequest] ([Year],ZoneCode, BranchCode, UserTypeID, TotalReqQty,isActive,CreatedBy,CreatedOn,ECRiderCode) values(@Year,@zonecode,@BranchCode,1008,@TotalReqQty,1,@CreatedBy,GETDATE(),@ECRiderCode);
                                                        SELECT SCOPE_IDENTITY();", new { @Year = DateTime.Now.Year, @ZoneCode = 28, @BranchCode = HttpContext.Current.Session["BRANCHCODE"].ToString(), @TotalReqQty = model.StockRequestDetails.Sum(x => x.Qty), @CreatedBy = HttpContext.Current.Session["U_ID"].ToString(), ECRiderCode = model.StockRequestDetails.First().DepartmentID }, transaction: tran);
