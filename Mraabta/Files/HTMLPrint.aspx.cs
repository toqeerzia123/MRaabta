...
{
    public partial class HTMLPrint : System.Web.UI.Page
    {
        ...
        public void PrintLoadSheet()
        {
            ...
            if (cn != null)
            {
                ...
                foreach (DataRow row in mrDt.Rows)
                {
                    ...
                    dr["consignee"] = row["consignee"].ToString();
                    DataRow drr = Dt.Select("consignmentNumber = '" + row["consignmentNumber"].ToString() + "'").FirstOrDefault();
                    if (drr != null)
                    {
                        dr["TransferInvoice"] = drr["couponNumber"].ToString();
                        dr["Destination"] = row["dest"].ToString(); // + '-' + drr["LOCATIONNAME"].ToString();
                    }
                    dt.Rows.Add(dr);
                }
        ...
        {
            string sqlString = @" SELECT c.consignmentNumber, c.couponNumber, b.name destination, cpl.LocationName,  C.bookingDate
  FROM Consignment c 
  INNER JOIN branches b ON b.branchCode= c.destination
  left JOIN CustomerPickupLocations cpl ON cpl.Id= c.locationID WHERE c.consignmentNumber IN (" + cns + ")";
        ...
        public void PrintReport(DataTable dt)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if ((rowCount + additionalRows) >= 45 && i > 0)
                {
                    html.Append("</table>");
                    html.Append("</div>");
        ...
        public string HeaderRow()
        {
            ...
            "                </td>\n" +
            "                <td style=\"width: 13%; border: 2px Solid Black; text-align:center;\">\n" +
            "                    <span style='font-size:small; font-weight:bold;'><b>Consignee Name</b></span>\n" +
            "                </td>\n" +
            "                <td style=\"width: 13%; border: 2px Solid Black; text-align:center;\">\n" +
            "                    <span style='font-size:small; font-weight:bold;'><b>Booking DateTime</b></span>\n" +
            "                </td>\n" +
        ...
        public string DataRow(DataRow dr, string rowStyle)
        {
            ...
            "                </td>\n" +
            "                <td style=\"width: 13%; border: 1px Solid Black; text-align:center;\">\n" +
            "                    <span style='font-size:small'> " + dr["consignee"].ToString() + "</span>\n" +
            "                </td>\n" +
            "                <td style=\"width: 13%; border: 1px Solid Black; text-align:center;\">\n" +
